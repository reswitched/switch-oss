...
#else
    static LazyNeverDestroyed<HashSet<String, ASCIICaseInsensitiveHash>> features;
    if (features.isNull())
        features.construct([] {
#endif
...
#if PLATFORM(WKC)
    if (requiredFeatures.isNull())
        requiredFeatures.construct(createSVGTestPropertyInfo(requiredFeaturesAttr, SVGElement::synchronizeRequiredFeatures));
#endif
...
#if PLATFORM(WKC)
    if (requiredExtensions.isNull())
        requiredExtensions.construct(createSVGTestPropertyInfo(requiredExtensionsAttr, SVGElement::synchronizeRequiredExtensions));
#endif
...
#if PLATFORM(WKC)
    if (systemLanguage.isNull())
        systemLanguage.construct(createSVGTestPropertyInfo(systemLanguageAttr, SVGElement::synchronizeSystemLanguage));
#endif
...
#if PLATFORM(WKC)
    if (map.isNull())
        map.construct(createSVGTextAttributeToPropertyMap());
#endif
    return map;
}
