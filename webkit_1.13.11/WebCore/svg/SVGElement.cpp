...
#if PLATFORM(WKC)
    if (map.isNull())
        map.construct(createAttributeNameToAnimatedPropertyTypeMap());
#endif
    return map;
}
...
#if PLATFORM(WKC)
    if (map.isNull())
        map.construct(createCSSPropertyWithSVGDOMNameToAnimatedPropertyTypeMap());
#endif
    return map;
}
...
#else
    static LazyNeverDestroyed<HashMap<AtomicString, QualifiedName>> animatableAttributes;
