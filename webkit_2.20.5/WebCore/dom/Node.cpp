...
LayoutRect Node::absoluteCompositeTransformedRectInternal(bool clip) const
{
    ...
    if (!renderer()->absoluteBoundingBoxRect(false).isEmpty()) {
        if (clip) {
            rect = renderer()->absoluteClippedOverflowRect();
    ...
    for (Element* element = ElementTraversal::firstWithin((Element &)*this); element; element = ElementTraversal::nextSibling(*element)) {
        ...
        rect.unite(element->absoluteCompositeTransformedRectInternal(true));
    }

    return rect;
}

LayoutRect Node::absoluteCompositeTransformedRect(bool ignoreBorder) const
{
    LayoutRect rect = absoluteCompositeTransformedRectInternal(false);
...
LayoutRect Node::absoluteClippedCompositeTransformedRect(bool ignoreBorder) const
{
    LayoutRect rect = absoluteCompositeTransformedRect(ignoreBorder);
...
#if PLATFORM(WKC)
WKC_DEFINE_GLOBAL_TYPE_ZERO(Lock*, s_eventTargetDataMapLock);
#else
...
#if PLATFORM(WKC)
    if (!s_eventTargetDataMapLock)
        s_eventTargetDataMapLock = new Lock();
