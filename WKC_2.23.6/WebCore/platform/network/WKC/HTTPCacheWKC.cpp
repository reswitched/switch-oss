...
bool HTTPCachedResource::readFile(const String& filepath)
{
    if (m_cacheReadBuffer)
        return true;

    String fileFullPath = pathByAppendingComponent(filepath, m_fileName);
    m_cacheReadBuffer = SharedBuffer::createWithContentsOfFile(fileFullPath);
    if (!m_cacheReadBuffer)
        return false;
    return true;
}
...
void HTTPCachedResource::setResourceData(SharedBuffer* resourceData)
{
    ...
    if (!resourceData) {
        ...
        resourceData->data(); // Calling data() in the network thread is dangerous, so call it here beforehand.
...
void HTTPCache::reset()
{
    ...
    for (size_t num = 0; num < m_resourceList.size(); num++) {
        ...
        LOG_FUNCTION(HTTPCache, "delete HTTPCachedResource: %p", resource);
        delete resource;
    }
    m_resourceList.clear();
    m_resources.clear();
...
void HTTPCache::removeResourceByNumber(int listNumber)
{
    ...
    LOG_FUNCTION(HTTPCache, "remove cache file %s (HTTPCachedResource: %p)", resource->fileName().utf8().data(), resource);
...
bool HTTPCache::write(HTTPCachedResource *resource)
{
    ...
    if (resource->resourceSize() > m_maxContentEntryInfoSize) {
        LOG_FUNCTION(HTTPCache, "entry info size over (HTTPCachedResource: %p)", resource);
        return false;
    }

    if (m_resourceList.size() >= m_maxContentEntries) {
        if (purgeOldest()==0) {
            LOG_FUNCTION(HTTPCache, "entry limit over (HTTPCachedResource: %p)", resource);
            return false;
        }
    }
    ...
    if (gCanCacheToDiskCallback && !gCanCacheToDiskCallback(resource->url().utf8().data(),
        ...
        LOG_FUNCTION(HTTPCache, "CanCacheToDiskCallback returned false (HTTPCachedResource: %p)", resource);
        return false;
    }
    ...
    if (m_totalContentsSize + alignedContentLength > m_maxTotalContentsSize) {
        ...
        if (purgeBySize(sizeToRemove) < sizeToRemove) {
            LOG_FUNCTION(HTTPCache, "exceeded the total size (HTTPCachedResource: %p)", resource);
            return false;
        }
    }

    if (!resource->writeFile(m_filePath)) {
        LOG_FUNCTION(HTTPCache, "writeFile failed (HTTPCachedResource: %p)", resource);
        return false;
    }
...
bool HTTPCache::read(HTTPCachedResource *resource)
{
    if (!resource->getResourceData()) {
        if (!resource->readFile(m_filePath)) {
    ...
    updateResourceLastUsedTime(resource);
    resource->setUsed(false);
    LOG_FUNCTION(HTTPCache, "cache read %s (HTTPCachedResource: %p)", resource->url().utf8().data(), resource);
    return true;
}
...
bool HTTPCache::readFATFile()
{
    ...
    if (!rv.getValue(buf)) {
        LOG_FUNCTION(HTTPCache, "FAT file read error: no memory");
        goto error_end;
    }
    ...
    if (!verifyDigest((unsigned char*)buffer, len)) {
        LOG_FUNCTION(HTTPCache, "FAT file read error: MD5 check");
        remove = true;
        goto error_end;
    }
