...
extern void RunLoop_setCycleProc(bool(*)(void*), void*);
}
...
//    settings->setAllowScriptsToCloseWindows(true);
    ...
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setLayoutFormattingContextEnabled(true);
#endif

#if ENABLE(INTERSECTION_OBSERVER)
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setIntersectionObserverEnabled(true);
#endif
...
void
WKCWebView::executeScriptIgnoringException(const char* script, bool forceUserGesture)
{
    m_private->core()->mainFrame().script().executeScriptIgnoringException(WTF::String::fromUTF8(script), forceUserGesture);
}

void
WKCWebView::executeUserAgentScriptInWorldIgnoringException(DOMWrapperWorld& world, const char* script, bool forceUserGesture)
{
    if (world.priv().webcore())
        m_private->core()->mainFrame().script().executeUserAgentScriptInWorldIgnoringException(*world.priv().webcore(), WTF::String::fromUTF8(script), forceUserGesture);
}
...
public:
    ...
    void cancelWakeUp(void* in_timer) {
        m_timerEventHandler.cancelWakeUp(in_timer);
    };
private:
...
WKCWebKitInitialize(void* memory, size_t physical_memory_size, size_t virtual_memory_size, void* font_memory, size_t font_memory_size, WKCMemoryEventHandler& memory_event_handler, WKCTimerEventHandler& timer_event_handler)
{
    ...
    WebCore::atomCanonicalTextEncodingName("UTF-8");
...
WKCWebKitStartWebInspector(const char* addr, int port, bool(*modalcycle)(void*), void* opaque)
{
    ...
    if (success) {
        WTF::RunLoop_setCycleProc(modalcycle, opaque);
...
WKCWebView::getScrollableContentRect(const WKC::Node* scrollableNode)
{
    ...
    if (WTF::is<WebCore::HTMLFrameOwnerElement>(node)) {
        ...
        if (!frameOwnerElement->renderer() || !frameOwnerElement->contentFrame() || frameOwnerElement->scrollingMode() == WebCore::ScrollbarAlwaysOff) {
            return WKCFloatRect();
        }
...
#if !LOG_DISABLED || !RELEASE_LOG_DISABLED
    if (logLevelString)
        WebCore::initializeLogChannelsIfNecessary(WTF::String(logLevelString));
    else
        WebCore::initializeLogChannelsIfNecessary(WTF::String("-all"));
#endif
}
