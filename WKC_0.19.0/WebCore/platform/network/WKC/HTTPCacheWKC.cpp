...
double HTTPCachedResource::computeCurrentAge()
{
    ...
    double residentTime = currentTime() - m_responseTime;
...
bool HTTPCachedResource::setResourceResponse(const ResourceResponse &response, bool setresponsetime)
{
    ...
    m_expires = response.expires() ? (double)response.expires().value().get().count() / 1000 / 1000 : numeric_limits<double>::quiet_NaN();
    m_date = response.date() ? (double)response.date().value().get().count() / 1000 / 1000 : numeric_limits<double>::quiet_NaN();
    m_lastModified = response.lastModified() ? (double)response.lastModified().value().get().count() / 1000 / 1000 : numeric_limits<double>::quiet_NaN();
    ...
    if (!(m_httpequivflags&EHTTPEquivMaxAge))
        m_maxAge = response.cacheControlMaxAge() ? (double)response.cacheControlMaxAge().value().count() / 1000 / 1000 : numeric_limits<double>::quiet_NaN();
