...
#include "WebKit/Storage/WebStorageNamespaceProvider.h"
...
WKC_DEFINE_GLOBAL_PTR(WTF::Vector<WKCWebViewPrivate*>*, gValidViews, 0);
       ...
       m_clientBuilders(builders)
{
    ...
    m_viewState = WebCore::ViewState::NoFlags;
...
WKCWebViewPrivate::construct()
{
...
#if USE(WKC_CUSTOM_BACKFORWARD_LIST)
...
#endif
#if ENABLE(BATTERY_STATUS)
    batteryclient = WKC::BatteryClientWKC::create(this);
    if (!batteryclient) goto error_end;
#endif
#if ENABLE(NOTIFICATIONS)
...
#endif
#if ENABLE(VIBRATION)
    vibrationClient = WKC::VibrationClientWKC::create(this);
#endif
    ...
    cli.userContentController = WebCore::UserContentController::create();
...
#endif
#if ENABLE(BATTERY_STATUS)
    provideBatteryTo(m_corePage, batteryclient);
#endif
#if ENABLE(NOTIFICATIONS)
...
#if ENABLE(VIBRATION)
    if (vibrationClient)
        provideVibrationTo(m_corePage, vibrationClient);
#endif
...
//    settings->setAllowScriptsToCloseWindows(true);
    ...
    WebCore::WebSocket::setIsAvailable(true);
     ...
     WebCore::RuntimeEnabledFeatures::sharedFeatures().setWebkitVideoTrackEnabled(true);
...
#endif
#if ENABLE(CSP_NEXT)
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setExperimentalContentSecurityPolicyFeaturesEnabled(true);
#endif
#if ENABLE(FONT_LOAD_EVENTS)
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setFontLoadEventsEnabled(true);
#endif
    ...
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setInputTypeDateEnabled(false);
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setInputTypeTimeEnabled(false);
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setInputTypeMonthEnabled(false);
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setInputTypeWeekEnabled(false);
    WebCore::RuntimeEnabledFeatures::sharedFeatures().setInputTypeDateTimeLocalEnabled(false);
#endif

    m_corePage->setSessionID(WebCore::SessionID::defaultSessionID());
...
{
    if (isVisible) {
        m_viewState &= ~WebCore::ViewState::IsVisuallyIdle;
        m_viewState |= WebCore::ViewState::IsVisible | WebCore::ViewState::IsVisibleOrOccluded;
...
{
    m_viewState |= WebCore::ViewState::WindowIsActive | WebCore::ViewState::IsFocused;
    core()->setViewState(m_viewState);
}
...
{
    // not suppored when USE(WKC_CUSTOM_BACKFORWARD_LIST) defined.
    return;
}
...
#else // USE(WKC_CUSTOM_BACKFORWARD_LIST)

void
WKCWebView::setMaintainsBackForwardList(bool flag)
{
    m_private->wkcCore()->backForwardList()->priv().webcore()->setEnabled(flag);
}

void
WKCWebView::addHistoryItem(const char* uri, const unsigned short* title, const WKCPoint* scrollPoint)
{
    WebCore::BackForwardClient* client;

    client = m_private->core()->backForward().client();
    if (!client)
        return;

    Ref<WebCore::HistoryItem> item = WebCore::HistoryItem::create(WTF::String::fromUTF8(uri), WTF::String(title));
    if (scrollPoint)
        item->setScrollPoint(WebCore::IntPoint(*scrollPoint));
    client->addItem(item.copyRef());
}

unsigned int
WKCWebView::getHistoryLength()
{
    WebCore::BackForwardList* list = m_private->wkcCore()->backForwardList()->priv().webcore();
...
WKCWebView::getHistoryIndexByItem(WKC::HistoryItem* item, unsigned int& index)
{
    ...
    for (i=0; i < historyItems.size(); ++i) {
        if (historyItems[i].get().url() == item->priv().webcore()->url()) {
            index = i;
            return true;
        }
    }
...
WKCWebView::getHistoryItemByIndex(unsigned int index, char* const uri, unsigned int& uriLen, unsigned short* const title, unsigned int& titleLen)
{
    ...
    uriLen = item->urlString().utf8().length() + 1;

    WKC::String s = item->title().utf8().data();
    if (title) {
        wkc_wstrncpy(title, 0, s.characters(), s.length());
        *(title + s.length()) = 0;
    }
    titleLen = s.length() + 1;
...
WKCWebView::gotoHistoryItemByIndex(unsigned int index)
{
    ...
    m_private->core()->goToItem(*item, WebCore::FrameLoadType::IndexedBackForward);
}

#endif // USE(WKC_CUSTOM_BACKFORWARD_LIST)
...
{
    if (wkcMemoryGetAllocationStatePeer() == WKC_MEMORYALLOC_TYPE_ANIMATED_IMAGE) {
        /* The GIF / APNG decoder is deleted in use. It cause the access violation. */
        return;
    }
    ...
    if (type & EMemoryTypeNoncriticalStylePresentationAttributeCache) {
    ...
    if (type & EMemoryTypeCriticalStyleResolver) {
        WTF::Vector<RefPtr<WebCore::Document>> documents;
        WTF::copyToVector(WebCore::Document::allDocuments(), documents);
    ...
    if (type & EMemoryTypeCriticalJITCompiledCode) {
        WebCore::GCController::singleton().discardAllCompiledCode();
    }
    ...
    if (type & EMemoryTypeCriticalIconDatabase) {
        clearIconDatabase();
    }
...
WKCWebView::draggableFromPoint(int x, int y)
{
    ...
    while (element) {
        ...
        if (element->isHTMLElement() && element->hasTagName(WebCore::HTMLNames::inputTag)) {
            WebCore::HTMLInputElement* ie = element->toInputElement();
...
bool WKCWebView::addVisitedLink(const char* uri, const unsigned short* title, const struct tm* date)
{
    ...
    WebCore::LinkHash hash = WebCore::visitedLinkHash(url);
...
public:
    ...
    void* notifyMemoryExhaust(size_t request_size, size_t& allocated_size) {
    ...
    void notifyStackOverflow(bool need_restart, size_t stack_size, size_t consumption, size_t margin, void* stack_top, void* stack_base, void* current_stack_top, const char* file, int line, const char* function) {
...
WKCWebKitConvertAllocationReason(int in_reason)
{
    ...
    case WKC_MEMORYALLOC_TYPE_IMAGE:
        ...
        break;
    case WKC_MEMORYALLOC_TYPE_ANIMATED_IMAGE:
        result = WKCMemoryEventHandler::AnimatedImage;
        break;
...
static void
WKCWebKitNotifyStackOverflow(bool need_restart, size_t stack_size, size_t consumption, size_t margin, void* stack_top, void* stack_base, void* current_stack_top, const char* file, int line, const char* function)
{
    if (need_restart) {
        WKCWebkitPrepareRestart();
    }
...
WKCWebKitInitialize(void* memory, size_t physical_memory_size, size_t virtual_memory_size, void* font_memory, size_t font_memory_size, WKCMemoryEventHandler& memory_event_handler, WKCTimerEventHandler& timer_event_handler)
{
    ...
    setPlatformStrategies(new WKC::PlatformStrategiesWKC());
...
#endif
    JSC::VM& vm = WebCore::JSDOMWindowBase::commonVM();
...
WKCWebView::isScrollableNode(const WKC::Node* node)
{
    ...
    if (WebCore::RenderObject* renderer = coreNode->renderer()) {
        ...
        if (renderer->isTextArea()) {
            ...
            WebCore::TextControlInnerTextElement* innerTextElement = form.innerTextElement();
...
{
#if ENABLE(GAMEPAD)
    WebCore::GamepadProviderWKC::singleton().updateGamepadValue(index, timestamp, naxes, axes, nbuttons, buttons);
    return true;
#else
    return false;
#endif
