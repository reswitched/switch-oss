...
#define DEFAULT_WATCHDOG_INTERVAL Seconds(20)
Seconds gWatchdogInterval = DEFAULT_WATCHDOG_INTERVAL;

void
WKCWebKitSetWatchdogInterval(int in_interval_ms)
{
    gWatchdogInterval = Seconds::fromMilliseconds(in_interval_ms);
}

static void setWatchdogTimeLimit();
...
shouldInterruptJavaScript(JSC::ExecState*, void*, void*)
{
    if (!gValidViews || !gValidViews->size() || !gValidViews->first() || !gValidViews->first()->core())
        return true;
    bool ret = gValidViews->first()->core()->chrome().client().shouldInterruptJavaScript();
    if (!ret)
        setWatchdogTimeLimit();
    return ret;
}
...
setWatchdogTimeLimit()
{
    ...
    if (!vm.watchdog())
        vm.ensureWatchdog();
    vm.watchdog()->setTimeLimit(gWatchdogInterval, shouldInterruptJavaScript, 0, 0);
}
