...
#else
    static LazyNeverDestroyed<HashSet<AtomString>> set;
    if (set.isNull())
        set.construct([] {
#endif
...
#else
    static LazyNeverDestroyed<HashSet<AtomString>> map;
    if (map.isNull())
        map.construct(HashSet<AtomString> {
#endif
        "American Typewriter",
        "Arial Hebrew",
        "Chalkboard",
...
#if (!PLATFORM(WIN) || USE(DIRECT2D)) && !PLATFORM(WKC)
