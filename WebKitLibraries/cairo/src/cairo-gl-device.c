...
#if defined(_MSC_VER) && defined(WKC_CAIRO_CUSTOMIZE)
void APIENTRY glActiveTexture(GLenum texture)
{
    static void(*func)(GLenum) = 0;
    if (!func) {
        func = wglGetProcAddress("glActiveTexture");
    }
    func(texture);
}
...
{
    static void(*func)(GLenum, GLenum, GLenum, GLenum) = 0;
    if (!func) {
        func = wglGetProcAddress("glBlendFuncSeparate");
    }
    func(sfactorRGB, dfactorRGB, sfactorAlpha, dfactorAlpha);
}
#endif
