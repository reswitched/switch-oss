...
std::pair<Image*, float> CachedImage::brokenImage(float deviceScaleFactor) const
{
...
#else
        WKC_DEFINE_STATIC_PTR(Image*, brokenImageVeryHiRes, Image::loadPlatformResource("missingImage@3x").leakRef());
#endif
        return std::make_pair(brokenImageVeryHiRes, 3.f);
    }

    if (deviceScaleFactor >= 2) {
#if !PLATFORM(WKC)
        static NeverDestroyed<Image*> brokenImageHiRes(Image::loadPlatformResource("missingImage@2x").leakRef());
        return std::make_pair(brokenImageHiRes, 2.f);
#else
        WKC_DEFINE_STATIC_PTR(Image*, brokenImageHiRes, Image::loadPlatformResource("missingImage@2x").leakRef());
        return std::make_pair(brokenImageHiRes, 2.f);
#endif
    }
    ...
    return std::make_pair(brokenImageLoRes, 1.f);
#else
    WKC_DEFINE_STATIC_PTR(Image*, brokenImageLoRes, Image::loadPlatformResource("missingImage").leakRef());
    float scale = 1.f / Image::platformResourceScale();
    return std::make_pair(brokenImageLoRes, scale);
#endif
}
