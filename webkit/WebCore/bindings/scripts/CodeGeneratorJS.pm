...
sub GenerateHeader
{
    ...
    if (!$hasParent ||
        ...
        if ($interfaceName ne "Node" && $codeGenerator->InheritsInterface($interface, "Node")) {
            ...
            push(@headerContent, "WTF_MAKE_FAST_ALLOCATED;\n");
        } else {
        ...
        push(@headerContent, "    WKC_DEFINE_STATIC_PTR(JS${interfaceName}Owner*, owner, 0);\n");
        push(@headerContent, "    if (!owner)\n");
        push(@headerContent, "        owner = new JS${interfaceName}Owner();\n");
        push(@headerContent, "     return owner;\n");
        push(@headerContent, "#endif\n");
        push(@headerContent, "}\n");
        push(@headerContent, "\n");
    }
    ...
    foreach my $parameter (@{$function->parameters}) {
        ...
        if ($argType eq "XPathNSResolver") {
            ...
            if ($parameter->extendedAttributes->{"RequiresExistingAtomicString"}) {
                ...
                if ($parameter->type eq "any" or $parameter->type eq "Dictionary") {
                    push(@$outputArray, "    " . GetNativeTypeFromSignature($parameter) . " $name" . JSValueToNative($parameter, $inner, $function->signature->extendedAttributes->{"Conditional"}) . ";\n");
                } else {
...
sub GenerateHashTable
{
    ...
    push(@implContent, "WKC_DEFINE_GLOBAL_HASHTABLE($name, $packedSize, $compactSizeMask, $hasSetter, $nameEntries, 0, $nameIndex);\n");
    push(@implContent, "#else\n");
