...
#else
        std::function<void(ScriptExecutionContext&)> p(std::allocator_arg, WTF::voidScriptExecutionContextFuncAllocator(), [successCallback, protectedStream](ScriptExecutionContext&) {
            successCallback->handleEvent(protectedStream.get());
        });
        document->postTask(p);
#endif
...
#else
        std::function<void(ScriptExecutionContext&)> p(std::allocator_arg, WTF::voidScriptExecutionContextFuncAllocator(), [errorCallback, protectedError](ScriptExecutionContext&) {
            errorCallback->handleEvent(protectedError.get());
        });
        document->postTask(p);
#endif
