...
#else
    WKC_DEFINE_STATIC_PTR(CSSParserContext*, strictContext, new CSSParserContext(CSSStrictMode));
    return *strictContext;
#endif
}
...
#if PLATFORM(WKC)
    if (m_dataStart16)
        WTF::fastFree(m_dataStart16.release());
    if (m_dataStart8)
        WTF::fastFree(m_dataStart8.release());
#endif
}
...
#else
        LChar* p = (LChar *)WTF::fastMalloc(sizeof(LChar) * length);
        if (m_dataStart8)
            WTF::fastFree(m_dataStart8.release());
        m_dataStart8 = std::unique_ptr<LChar[]> (p);
#endif
...
#else
    UChar* p = (UChar *)WTF::fastMalloc(sizeof(UChar) * length);
    if (m_dataStart16)
        WTF::fastFree(m_dataStart16.release());
    m_dataStart16 = std::unique_ptr<UChar[]> (p);
#endif
    for (unsigned i = 0; i < m_parsedTextPrefixLength; ++i)
...
#if PLATFORM(WKC) && defined(_DEBUG)
    if (shorthands.size() == 0)
        return;
#endif
...
UChar*& CSSParser::currentCharacter16()
{
    if (!m_currentCharacter16) {
        ...
        m_currentCharacter16 = m_dataStart16.get();
#else
        m_currentCharacter16 = (UChar *)WTF::fastMalloc(sizeof(UChar) * m_length);
        m_dataStart16 = std::unique_ptr<UChar[]>(m_currentCharacter16);
#endif
    }
...
#else
    WKC_DEFINE_STATIC_PTR(const char*, appleWirelessPlaybackTargetActive, getValueName(CSSValueAppleWirelessPlaybackTargetActive));
#endif
