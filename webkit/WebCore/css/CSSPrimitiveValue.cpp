...
static CSSTextCache& cssTextCache()
{
    ...
    return cache;
#else
    WKC_DEFINE_STATIC_PTR(CSSTextCache*, cache, 0);
    if (!cache)
        cache = new CSSTextCache();
    return *cache;
#endif
}
...
#else
    WKC_DEFINE_STATIC_PTR(AtomicString*, keywordStrings, new AtomicString[numCSSValueKeywords]); // Leaked intentionally.
#endif
