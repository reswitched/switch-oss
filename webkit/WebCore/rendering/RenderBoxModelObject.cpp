...
static ContinuationMap& continuationMap()
{
    ...
    return map;
#else
    WKC_DEFINE_STATIC_PTR(ContinuationMap*, map, 0);
    if (!map)
        map = new ContinuationMap();
    return *map;
#endif
}
...
#else
WKC_DEFINE_GLOBAL_PTR(FirstLetterRemainingTextMap*, firstLetterRemainingTextMap, nullptr);
#endif
...
LayoutPoint RenderBoxModelObject::adjustedPositionRelativeToOffsetParent(const LayoutPoint& startPoint) const
{
    ...
    if (const RenderBoxModelObject* offsetParent = this->offsetParent()) {
        ...
        if (!isOutOfFlowPositioned() || flowThreadContainingBlock()) {
            ...
            // Since we will bypass the bodyÅfs renderer anyway, just end the loop if we encounter a region flow (named flow thread).
...
LayoutSize RenderBoxModelObject::calculateFillTileSize(const FillLayer& fillLayer, const LayoutSize& positioningAreaSize) const
{
    ...
    switch (type) {
        ...
        case SizeNone: {
            // If both values are ÅeautoÅf then the intrinsic width and/or height of the image should be used, if any.
            if (!imageIntrinsicSize.isEmpty())
                return imageIntrinsicSize;

            // If the image has neither an intrinsic width nor an intrinsic height, its size is determined as for ÅecontainÅf.
            type = Contain;
        }
        FALLTHROUGH;
        case Contain:
