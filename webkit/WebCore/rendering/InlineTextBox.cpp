...
static void drawSkipInkUnderline(TextPainter& textPainter, GraphicsContext& context, FloatPoint localOrigin, float underlineOffset, float width, bool isPrinting, bool doubleLines, StrokeStyle strokeStyle)
{
...
void InlineTextBox::paintDecoration(GraphicsContext& context, const FloatPoint& boxOrigin, TextDecoration decoration, const ShadowData* shadow, TextPainter& textPainter)
{
    ...
    do {
        ...
        if (decoration & TextDecorationUnderline) {
            StrokeStyle strokeStyle = textDecorationStyleToStrokeStyle(underlineStyle);
            ...
            default:
                ...
                if ((lineStyle.textDecorationSkip() == TextDecorationSkipInk || lineStyle.textDecorationSkip() == TextDecorationSkipAuto) && isHorizontal()) {
                    if (!context.paintingDisabled()) {
                        drawSkipInkUnderline(textPainter, context, localOrigin, underlineOffset, width, isPrinting, underlineStyle == TextDecorationStyleDouble, strokeStyle);
                    }
                } else
                    // FIXME: Need to support text-decoration-skip: none.
#endif // CSS3_TEXT_DECORATION_SKIP_INK
                    context.drawLineForText(FloatPoint(localOrigin.x(), localOrigin.y() + underlineOffset), width, isPrinting, underlineStyle == TextDecorationStyleDouble, strokeStyle);
            }
        }
        if (decoration & TextDecorationOverline) {
            StrokeStyle strokeStyle = textDecorationStyleToStrokeStyle(overlineStyle);
            ...
            default:
                ...
                if ((lineStyle.textDecorationSkip() == TextDecorationSkipInk || lineStyle.textDecorationSkip() == TextDecorationSkipAuto) && isHorizontal()) {
                    if (!context.paintingDisabled())
                        drawSkipInkUnderline(textPainter, context, localOrigin, 0, width, isPrinting, overlineStyle == TextDecorationStyleDouble, strokeStyle);
                } else
                    // FIXME: Need to support text-decoration-skip: none.
#endif // CSS3_TEXT_DECORATION_SKIP_INK
                    context.drawLineForText(localOrigin, width, isPrinting, overlineStyle == TextDecorationStyleDouble, strokeStyle);
            }
        }
        if (decoration & TextDecorationLineThrough) {
            StrokeStyle strokeStyle = textDecorationStyleToStrokeStyle(linethroughStyle);
            ...
            default:
                ...
                if ((lineStyle.textDecorationSkip() == TextDecorationSkipInk || lineStyle.textDecorationSkip() == TextDecorationSkipAuto) && isHorizontal()) {
                    if (!context.paintingDisabled())
                        drawSkipInkUnderline(textPainter, context, localOrigin, 2 * baseline / 3, width, isPrinting, linethroughStyle == TextDecorationStyleDouble, strokeStyle);
                }
                else
                    // FIXME: Need to support text-decoration-skip: none.
#endif // CSS3_TEXT_DECORATION_SKIP_INK
#endif // PLATFORM(WKC)
                    context.drawLineForText(FloatPoint(localOrigin.x(), localOrigin.y() + 2 * baseline / 3), width, isPrinting, linethroughStyle == TextDecorationStyleDouble, strokeStyle);
            }
        }
    } while (shadow);
