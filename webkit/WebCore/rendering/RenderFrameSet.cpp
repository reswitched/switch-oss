...
bool RenderFrameSet::userResize(MouseEvent* evt)
{
    ...
    if (!m_isResizing) {
        if (needsLayout())
            return false;
        if (evt->type() == eventNames().mousedownEvent && evt->button() == (unsigned short)LeftButton) {
        ...
        if (evt->type() == eventNames().mousemoveEvent || (evt->type() == eventNames().mouseupEvent && evt->button() == (unsigned short)LeftButton)) {
            FloatPoint localPos = absoluteToLocal(evt->absoluteLocation(), UseTransforms);
            continueResizing(m_cols, localPos.x());
            continueResizing(m_rows, localPos.y());
            if (evt->type() == eventNames().mouseupEvent && evt->button() == (unsigned short)LeftButton) {
                setIsResizing(false);
                return true;
            }
        }
