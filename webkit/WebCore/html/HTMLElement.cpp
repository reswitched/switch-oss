...
#else
    EventHandlerNameMap* map = new EventHandlerNameMap();
#endif
...
#else
    WKC_DEFINE_STATIC_PTR(EventHandlerNameMap*, map, createEventHandlerNameMap());
...
static Ref<DocumentFragment> textToFragment(Document& document, const String& text, ExceptionCode& ec)
{
    ec = 0;
...
static inline const AtomicString& toValidDirValue(const AtomicString& value)
{
    ...
    if (equalIgnoringCase(value, autoValue))
        return autoValue;
#else
    WKC_DEFINE_STATIC_PTR(AtomicString*, ltrValue, 0);
    WKC_DEFINE_STATIC_PTR(AtomicString*, rtlValue, 0);
    WKC_DEFINE_STATIC_PTR(AtomicString*, autoValue, 0);
    if (!ltrValue)
        ltrValue = new AtomicString("ltr", AtomicString::ConstructFromLiteral);
    if (!rtlValue)
        rtlValue = new AtomicString("rtl", AtomicString::ConstructFromLiteral);
    if (!autoValue)
        autoValue = new AtomicString("auto", AtomicString::ConstructFromLiteral);
...
void HTMLElement::setInnerText(const String& text, ExceptionCode& ec)
{
    ...
    ec = 0;
    auto fragment = textToFragment(document(), text, ec);
...
void HTMLElement::setOuterText(const String& text, ExceptionCode& ec)
{
    ...
    if (text.contains('\r') || text.contains('\n'))
        newChild = textToFragment(document(), text, ec);
    else
