...
void HTMLSelectElement::listBoxDefaultEventHandler(Event* event)
{
    ...
    if (event->type() == eventNames().mousedownEvent && is<MouseEvent>(*event) && downcast<MouseEvent>(*event).button() == (unsigned short)LeftButton) {
        ...
        MouseEvent& mouseEvent = downcast<MouseEvent>(*event);
        if (mouseEvent.button() != (unsigned short)LeftButton || !mouseEvent.buttonDown())
            return;
    ...
    } else if (event->type() == eventNames().mouseupEvent && is<MouseEvent>(*event) && downcast<MouseEvent>(*event).button() == (unsigned short)LeftButton && document().frame()->eventHandler().autoscrollRenderer() != renderer()) {
