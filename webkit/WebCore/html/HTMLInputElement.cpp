...
void HTMLInputElement::willDispatchEvent(Event& event, InputElementClickState& state)
{
    ...
    if (event.type() == eventNames().clickEvent && is<MouseEvent>(event) && downcast<MouseEvent>(event).button() == (unsigned short)LeftButton) {
...
{
    ASSERT(evt);
    if (is<MouseEvent>(*evt) && evt->type() == eventNames().clickEvent && downcast<MouseEvent>(*evt).button() == (unsigned short)LeftButton) {
