...
#if PLATFORM(WKC)
    if (!imageData)
        wkcMemoryNotifyNoMemoryPeer(scaledSize.width() * scaledSize.height() * 4);
#endif
...
#if PLATFORM(WKC)
            if (!m_unmultipliedImageResult)
                wkcMemoryNotifyNoMemoryPeer(inputSize.width() * inputSize.height() * 4);
#endif
...
#if PLATFORM(WKC)
    if (!m_unmultipliedImageResult)
        wkcMemoryNotifyNoMemoryPeer(resultSize.width() * resultSize.height() * 4);
#endif
