...
#if PLATFORM(WKC)
    if (css.isNull())
        css.construct("CSS", AtomicString::ConstructFromLiteral);
#endif
    static NeverDestroyed<const AtomicString> xml("XML", AtomicString::ConstructFromLiteral);
#if PLATFORM(WKC)
    if (xml.isNull())
        xml.construct("XML", AtomicString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC)
    if (sum.isNull())
        sum.construct("sum", AtomicString::ConstructFromLiteral);
#endif
...
#if PLATFORM(WKC)
    if (inherit.isNull())
        inherit.construct("inherit", AtomicString::ConstructFromLiteral);
#endif
