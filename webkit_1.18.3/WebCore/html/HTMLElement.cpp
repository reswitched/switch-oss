...
#if PLATFORM(WKC)
    if (map.isNull())
        map.construct(createEventHandlerNameMap());
#endif
...
#if PLATFORM(WKC)
    if (ltrValue.isNull())
        ltrValue.construct("ltr", AtomicString::ConstructFromLiteral);
#endif
    static NeverDestroyed<AtomicString> rtlValue("rtl", AtomicString::ConstructFromLiteral);
#if PLATFORM(WKC)
    if (rtlValue.isNull())
        rtlValue.construct("rtl", AtomicString::ConstructFromLiteral);
#endif
    static NeverDestroyed<AtomicString> autoValue("auto", AtomicString::ConstructFromLiteral);
#if PLATFORM(WKC)
    if (autoValue.isNull())
        autoValue.construct("auto", AtomicString::ConstructFromLiteral);
#endif
