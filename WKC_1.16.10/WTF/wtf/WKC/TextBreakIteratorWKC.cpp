...
characterBreakIterator(StringView str)
{
    ...
    bool ret = wkcTextBreakIteratorSetStringPeer(gIteratorCharacter, reinterpret_cast<const unsigned short*>(s), len);
    freeUChar(str, s, len);
    if (!ret)
        return 0;
...
cursorMovementIterator(StringView str)
{
    ...
    ensureUChar(str, s, len);
    bool ret = wkcTextBreakIteratorSetStringPeer(gIteratorCursorMovement, reinterpret_cast<const unsigned short*>(s), len);
    freeUChar(str, s, len);
    if (!ret)
        return 0;
...
wordBreakIterator(StringView str)
{
    ...
    ensureUChar(str, s, len);
    bool ret = wkcTextBreakIteratorSetStringPeer(gIteratorWord, reinterpret_cast<const unsigned short*>(s), len);
    freeUChar(str, s, len);
    if (!ret)
        return 0;
...
UBreakIterator*
acquireLineBreakIterator(StringView str, const AtomicString& /*locale*/, const UChar* priorContext, unsigned int priorContextLength, LineBreakIteratorMode mode)
{
    void* iterator = 0;
    ...
    ensureUChar(str, s, len);
    bool ret = wkcTextBreakIteratorSetStringPeer(iterator, reinterpret_cast<const unsigned short*>(s), len);
    freeUChar(str, s, len);
    if (!ret) {
        wkcTextBreakIteratorDeletePeer(iterator);
        return 0;
    ...
    if (priorContextLength){
        ret = wkcTextBreakIteratorSetPriorContextPeer(iterator, reinterpret_cast<const unsigned short*>(priorContext), priorContextLength);
        // ignore return value.
    }
    ...
    : m_iterator(nullptr)
{
    ...
    if (sv.is8Bit()) {
        ...
        wkcTextBreakIteratorSetStringPeer((void *)m_iterator, reinterpret_cast<const unsigned short*>(p), sv.length());
        WTF::fastFree(p);
    } else {
...
sentenceBreakIterator(StringView str)
{
    ...
    ensureUChar(str, s, len);
    bool ret = wkcTextBreakIteratorSetStringPeer(gIteratorSentence, reinterpret_cast<const unsigned short*>(s), len);
    freeUChar(str, s, len);
    if (!ret)
        return 0;
