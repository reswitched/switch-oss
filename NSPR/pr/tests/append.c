...
#ifdef NN_NINTENDO_SDK
    if (PR_MkDir("tmp", 0777) < 0) {
        /*  If this fails, log about it but keep going as it could be because
            the directory already exists.  */
        if (debug) {
            printf("PR_Mkdir() failed for creating tmp: %d\n", PR_GetError());
        }
    }
#endif
...
/* ---------------------------------------------------------------------- */
    ...
    for ( i = 0; i < addedBytes ; i++ ) {
        ...
        if ( sizeof(inBuf) != rv)  {
            if (debug) printf("PR_Read() failed: %d\n", PR_GetError());
            failedAlready = PR_TRUE;
            goto Finished;
        }
        sum += inBuf;
