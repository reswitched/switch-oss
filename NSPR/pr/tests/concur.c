...
PRIntn PR_CALLBACK Concur(PRIntn argc, char **argv)
{
    ...
    if (threads == NULL)
    {
        PR_fprintf(PR_STDERR, "Failed to alloc PRThread* for %u\n", range);
        ret = -1;
        goto _errExit;
    }
    ...
    while (--loops > 0)
    {
        for (cpus = 1; cpus <= range; ++cpus)
        {
            ...
            if (threads[cpus - 1] == NULL)
            {
                PR_fprintf(PR_STDERR, "Failed to create thread %d\n", cpus - 1);
                break;
            }
        }
        ...
        /*  Wait for the threads to all finish before we proceed
            so we do not exhaust thread limit on the platform.  */
...
#endif
{
    int ret;
    PR_STDIO_INIT();
    ret = PR_Initialize(Concur, argc, argv, 0);
    return ret;
}  /* main */
