...
char *TEST_DIR = "prdir";
char *SMALL_FILE_NAME = "prdir/prsmallf";
char *LARGE_FILE_NAME = "prdir/prlargef";
...
Serve_Client(void *arg)
{
    ...
    if (in_buf == NULL) {
        PR_fprintf(outFile,"prsocket_test: failed to alloc buffer struct\n");
        failed_already=1;
        goto exit;
    }
    ...
    for (j = 0; j < num_tcp_mesgs_per_connection; j++) {
        ...
        if (readn(sockfd, in_buf->data, bytes) < bytes) {
            PR_fprintf(outFile,"prsocket_test: ERROR - Serve_Client:readn\n");
            failed_already=1;
            goto exit;
        }
        ...
        if (j == num_tcp_mesgs_per_connection - 1)
            if (PR_Shutdown(sockfd, PR_SHUTDOWN_RCV) < 0) {
                PR_fprintf(outFile,"prsocket_test: ERROR - PR_Shutdown\n");
            }
#endif
        DPRINTF(("Serve_Client [0x%lx]: inbuf[0] = 0x%lx\n",PR_GetCurrentThread(),
            (*((int *) in_buf->data))));
        if (writen(sockfd, in_buf->data, bytes) < bytes) {
            PR_fprintf(outFile,"prsocket_test: ERROR - Serve_Client:writen\n");
            failed_already=1;
            goto exit;
        }
    }
...
#if (defined(_PR_PTHREADS) && !defined(_PR_DCETHREADS)) || (defined(WIN32) && !defined(NN_NINTENDO_SDK))
	switch(index %  4) {
		case 0:
...
TCP_Server(void *arg)
{
	...
	if ((sockfd = PR_OpenTCPSocket(server_domain)) == NULL) {
        PR_fprintf(outFile,"prsocket_test: PR_NewTCPSocket failed\n");
        goto exit;
    }
	...
	if (PR_SetNetAddr(PR_IpAddrAny, server_domain, TCP_SERVER_PORT,
									&netaddr) == PR_FAILURE) {
        PR_fprintf(outFile,"prsocket_test: PR_SetNetAddr failed\n");
        goto exit;
	}
    /*
    ...
    while (PR_Bind(sockfd, &netaddr) < 0) {
        ...
        PR_fprintf(outFile,"prsocket_test: ERROR - PR_Bind failed\n");
        perror("PR_Bind");
        failed_already=1;
        goto exit;
    }
    ...
    if (PR_Listen(sockfd, 32) < 0) {
        PR_fprintf(outFile,"prsocket_test: ERROR - PR_Listen failed\n");
        failed_already=1;
        goto exit;
    }

    if (PR_GetSockName(sockfd, &netaddr) < 0) {
        PR_fprintf(outFile,"prsocket_test: ERROR - PR_GetSockName failed\n");
        failed_already=1;
        goto exit;
    }
    ...
    DPRINTF(("TCP_Server: Accepting connection\n"));
        if ((newsockfd = PR_Accept(sockfd, addrp,
            ...
            PR_fprintf(outFile,"prsocket_test: ERROR - PR_Accept failed\n");
            goto exit;
        }
    DPRINTF(("TCP_Server: Accepted connection\n"));
        scp = PR_NEW(Serve_Client_Param);
        if (scp == NULL) {
            PR_fprintf(outFile,"prsocket_test: PR_NEW failed\n");
            goto exit;
        }
        ...
        t = create_new_thread(PR_USER_THREAD,
            ...
            0, i);
        if (t == NULL) {
            PR_fprintf(outFile,"prsocket_test: PR_CreateThread failed\n");
            failed_already=1;
            goto exit;
        }
...
UDP_Server(void *arg)
{
	...
	if ((sockfd = PR_OpenUDPSocket(server_domain)) == NULL) {
        PR_fprintf(outFile,"prsocket_test: PR_NewUDPSocket failed\n");
        failed_already=1;
        return;
    }
    memset(&netaddr, 0 , sizeof(netaddr));
...
TCP_Client(void *arg)
{
    ...
    for (i = 0; i < num_tcp_connections_per_client; i++) {
        if ((sockfd = PR_OpenTCPSocket(client_domain)) == NULL) {
            PR_fprintf(outFile,"prsocket_test: PR_OpenTCPSocket failed (%d, %ld, %ld)\n", i, PR_GetError(), PR_GetOSError());
            failed_already=1;
            return;
        }
        if (PR_Connect(sockfd, &netaddr,PR_INTERVAL_NO_TIMEOUT) < 0){
        	PR_fprintf(outFile, "PR_Connect failed: (%ld, %ld)\n",
        ...
        for (j = 0; j < num_tcp_mesgs_per_connection; j++) {
            ...
            if (writen(sockfd, out_buf->data, bytes) < bytes) {
                PR_fprintf(outFile,
                           "prsocket_test: ERROR - TCP_Client:writen (%d, %d, %ld, %ld)\n",
                           i,
                           j,
                           PR_GetError(),
                           PR_GetOSError());
            ...
            if (readn(sockfd, in_buf->data, bytes) < bytes) {
                PR_fprintf(outFile,
                           "prsocket_test: ERROR - TCP_Client:readn (%d, %d, %ld, %ld)\n",
                           i,
                           j,
                           PR_GetError(),
                           PR_GetOSError());
            ...
            if (memcmp(in_buf->data, out_buf->data, bytes) != 0) {
                PR_fprintf(outFile,
                           "prsocket_test: ERROR - data corruption (%d, %d, %ld, %ld)\n",
                           i,
                           j,
                           PR_GetError(),
                           PR_GetOSError());
...
UDP_Client(void *arg)
{
    ...
    if ((sockfd = PR_OpenUDPSocket(client_domain)) == NULL) {
        PR_fprintf(outFile,"prsocket_test: PR_OpenUDPSocket failed\n");
        failed_already=1;
        return;
    }
    ...
    if (cp->udp_connect) {
        if (PR_Connect(sockfd, &netaddr,PR_INTERVAL_NO_TIMEOUT) < 0){
            PR_fprintf(outFile,"prsocket_test: PR_Connect failed\n");
            failed_already=1;
            return;
        }
    }
    ...
    for (i = 0; i < num_udp_datagrams_per_client; i++) {
         ...
         */
        if (memcmp(in_buf->data, out_buf->data, bytes) != 0) {
            PR_fprintf(outFile,"prsocket_test: ERROR - UDP data corruption\n");
            failed_already=1;
            return;
        }
    }
...
TCP_Socket_Client_Server_Test(void)
{
    ...
    if (server_sem == NULL) {
        PR_fprintf(outFile,"prsocket_test: PR_NewSem failed\n");
        failed_already=1;
        return -1;
    }
    mon2 = PR_NewMonitor();
    if (mon2 == NULL) {
        PR_fprintf(outFile,"prsocket_test: PR_NewMonitor failed\n");
        failed_already=1;
        return -1;
    }
...
TransmitFile_Client(void *arg)
{
    ...
    if (small_buf == NULL) {
        PR_fprintf(outFile,"prsocket_test: failed to alloc buffer\n");
        failed_already=1;
        return;
    }
...
Serve_TransmitFile_Client(void *arg)
{
    ...
    if (local_small_file_fd == NULL) {
        PR_fprintf(outFile,"prsocket_test failed to open file for transmitting %s\n",
            SMALL_FILE_NAME);
        failed_already=1;
        goto done;
    }
...
TransmitFile_Server(void *arg)
{
    ...
    if (t == NULL) {
        PR_fprintf(outFile, "prsocket_test: run out of memory\n");
        failed_already=1;
        goto exit;
    }
    /*
    ...
    if ((sockfd = PR_OpenTCPSocket(PR_AF_INET)) == NULL) {
        PR_fprintf(outFile,"prsocket_test: PR_OpenTCPSocket failed\n");
        failed_already=1;
        goto exit;
    }
    memset(&netaddr, 0 , sizeof(netaddr));
...
#define TRANSMITFILE_BUF_SIZE    (4 * 1024 - 11)
    ...
    if (small_file_fd == NULL) {
        PR_fprintf(outFile,"prsocket_test failed to create/open file %s\n",
            SMALL_FILE_NAME);
        failed_already=1;
        rv = -1;
        goto done;
    ...
    buf = PR_NEW(file_buf);
    if (buf == NULL) {
        PR_fprintf(outFile,"prsocket_test failed to allocate buffer\n");
        failed_already=1;
        rv = -1;
        goto done;
    }
    ...
    if (small_file_addr == (void *) -1) {
        PR_fprintf(outFile,"prsocket_test failed to mmap file %s\n",
            SMALL_FILE_NAME);
        failed_already=1;
        rv = -1;
        goto done;
    ...
    if (small_file_header == NULL) {
        PR_fprintf(outFile,"prsocket_test failed to malloc header file\n");
        failed_already=1;
        rv = -1;
        goto done;
    }
    ...
    if (small_file_trailer == NULL) {
        PR_fprintf(outFile,"prsocket_test failed to malloc header trailer\n");
        failed_already=1;
        rv = -1;
        goto done;
    }
    ...
    /*  NNSDK doesn't like other threads re-opening the same file, so close
        the handles here.  */
    ...
    if ((PR_Delete(SMALL_FILE_NAME)) == PR_FAILURE) {
        PR_fprintf(outFile,"prsocket_test: failed to unlink file %s\n",
            SMALL_FILE_NAME);
        failed_already=1;
    }
    ...
    if ((PR_RmDir(TEST_DIR)) == PR_FAILURE) {
        PR_fprintf(outFile,"prsocket_test failed to rmdir %s: (%ld, %ld)\n",
...
#if !defined(WIN16) && !defined(SYMBIAN) /*&& !defined(NN_NINTENDO_SDK)*/
    /*
