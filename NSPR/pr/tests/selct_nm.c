...
#endif
{
 ...
 /* main test */
    ...
    if (debug_mode) {
		PR_fprintf(outFile, "This program tests PR_Select with sockets.  \n");
		PR_fprintf(outFile, " Normal operation are tested.\n\n");
	}

    /* Create two listening sockets */
    if ((listenSock1 = PR_NewTCPSocket()) == NULL) {
	PR_fprintf(outFile, "Can't create a new TCP socket\n");
	failed_already=1;
	goto exit_now;
    }
    addr.inet.family = PR_AF_INET;
	...
	PR_fprintf(outFile, "Can't bind socket\n");
	failed_already=1;
	goto exit_now;
    }
    if (PR_GetSockName(listenSock1, &addr) == PR_FAILURE) {
	PR_fprintf(outFile, "PR_GetSockName failed\n");
	failed_already=1;
	goto exit_now;
    }
    listenPort1 = PR_ntohs(addr.inet.port);
    if (PR_Listen(listenSock1, 5) == PR_FAILURE) {
	PR_fprintf(outFile, "Can't listen on a socket\n");
	failed_already=1;
	goto exit_now;
    }
...
failed_already=1;
    ...
    if (debug_mode) PR_fprintf(outFile, "%s", buf);
	...
	PR_fprintf(outFile, "can't create thread\n");
	failed_already=1;
	goto exit_now;
    }
    ...
    if (debug_mode) {
		PR_fprintf(outFile, "Two client threads are created.  Each of them will\n");
		PR_fprintf(outFile, "send data to one of the two ports the server is listening on.\n");
		PR_fprintf(outFile, "The data they send is the port number.  Each of them send\n");
		PR_fprintf(outFile, "the data five times, so you should see ten lines below,\n");
		PR_fprintf(outFile, "interleaved in an arbitrary order.\n");
	}
    /* set up the fd array */
    fds = fds0;
    other_fds = fds1;
	...
	if (retVal == -1) {
	    PR_fprintf(outFile, "PR_Select failed (%d, %d)\n", PR_GetError(),
		    PR_GetOSError());
	failed_already=1;
	    goto exit_now;
	}
	...
	for (j = 0; j < 2; j++) {
		...
		if (sock == NULL) {
		    PR_fprintf(outFile, "PR_Accept() failed\n");
		failed_already=1;
		    goto exit_now;
		}
	...
	for (j = 2; j < nfds; j++) {
		...
		if (nBytes == -1) {
		    PR_fprintf(outFile, "PR_Read() failed\n");
		failed_already=1;
		    goto exit_now;
		}
		/* Just to be safe */
		buf[127] = '\0';
		PR_Close(fds[j]);
		if (debug_mode) PR_fprintf(outFile, "The server received \"%s\" from a client\n", buf);
	    } else {
		PR_FD_SET(fds[j], &readFdSet);
        ...
        if (debug_mode) {
            PR_fprintf(outFile, "retVal %d, nEvents %d\n", retVal, nEvents);
        }
    ...
    if (debug_mode) PR_fprintf(outFile, "Test passed\n");
