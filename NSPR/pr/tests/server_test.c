...
SetServerState(char *waiter, PRInt32 state)
{
	...
	if (debug_mode) DPRINTF(outFile, "\t%s changed state to %d\n", waiter, state);
...
WaitServerState(char *waiter, PRInt32 state)
{
    ...
    if (debug_mode) DPRINTF(outFile, "\t%s waiting for state %d\n", waiter, state);

    while(!(ServerState & state))
        PR_WaitCondVar(ServerStateCV, PR_INTERVAL_NO_TIMEOUT);
    rv = ServerState;

    if (debug_mode) DPRINTF(outFile, "\t%s resuming from wait for state %d; state now %d\n", 
...
WorkerThreadFunc(void *_listenSock)
{
    ...
    if (debug_mode) DPRINTF(outFile, "\tServer buffer is %d bytes; %d data, %d netaddrs\n",
    ...
    if (!dataBuf)
        if (debug_mode) PR_fprintf(outFile, "\tServer could not malloc space!?\n");
    sendBuf = (char *)PR_MALLOC(_server_data *sizeof(char));
    if (!sendBuf)
        if (debug_mode) PR_fprintf(outFile, "\tServer could not malloc space!?\n");

    if (debug_mode) DPRINTF(outFile, "\tServer worker thread running\n");
    ...
    while(1) {
        ...
        if (debug_mode) DPRINTF(outFile, "\tServer thread going into accept\n");
        ...
        if (bytesRead < 0) {
            if (debug_mode) PR_fprintf(outFile, "\tServer error in accept (%d)\n", bytesRead);
            continue;
        }

        if (debug_mode) DPRINTF(outFile, "\tServer accepted connection (%d bytes)\n", bytesRead);
        ...
        if (workerThreadsBusy == workerThreads) {
            ...
            if (workerThreadsBusy == workerThreads) {
                ...
                if (!WorkerThread) {
                    if (debug_mode) PR_fprintf(outFile, "Error creating client thread %d\n", workerThreads);
                } else {
                    PR_AtomicIncrement(&workerThreads);
                    if (debug_mode) DPRINTF(outFile, "\tServer creates worker (%d)\n", workerThreads);
                }
            }
        ...
        while (bytesToRead) {
            bytesRead = PR_Recv(newSock, 
                                ...
                                PR_INTERVAL_NO_TIMEOUT);
            if (bytesRead < 0) {
                if (debug_mode) PR_fprintf(outFile, "\tServer error receiving data (%d)\n", bytesRead);
                continue;
            }
            if (debug_mode) DPRINTF(outFile, "\tServer received %d bytes\n", bytesRead);
        }
        ...
        if (bytesWritten != _server_data) {
            if (debug_mode) PR_fprintf(outFile, "\tError sending data to client (%d, %d)\n", 
                bytesWritten, PR_GetOSError());
        } else {
            if (debug_mode) DPRINTF(outFile, "\tServer sent %d bytes\n", bytesWritten);
        }	
...
ServerSetup(void)
{
    ...
    if ( (listenSocket = PR_NewTCPSocket()) == NULL) {
        if (debug_mode) PR_fprintf(outFile, "\tServer error creating listen socket\n");
    ...
    if ( PR_Bind(listenSocket, &serverAddr) == PR_FAILURE) {
        if (debug_mode) PR_fprintf(outFile, "\tServer error binding to server address: OS error %d\n",
                PR_GetOSError());
		else Test_Result(FAIL);
        PR_Close(listenSocket);
        return NULL;
    ...
    if ( PR_Listen(listenSocket, 128) == PR_FAILURE) {
        if (debug_mode) PR_fprintf(outFile, "\tServer error listening to server socket\n");
		else Test_Result(FAIL);
        PR_Close(listenSocket);
    ...
    if (!WorkerThread) {
        if (debug_mode) PR_fprintf(outFile, "error creating working thread\n");
        PR_Close(listenSocket);
        return NULL;
    }
    PR_AtomicIncrement(&workerThreads);
    if (debug_mode) DPRINTF(outFile, "\tServer created primordial worker thread\n");
...
ServerThreadFunc(void *unused)
{
    ...
    if (!listenSocket) {
        ...
        if (debug_mode) DPRINTF(outFile, "\tServer up\n");
...
ClientThreadFunc(void *unused)
{
    ...
    if (!sendBuf)
        if (debug_mode) PR_fprintf(outFile, "\tClient could not malloc space!?\n");
    ...
    while(numRequests > 0) {
        ...
        if ( (numRequests % 10) == 0 )
            if (debug_mode) PR_fprintf(outFile, ".");
        if (debug_mode) DPRINTF(outFile, "\tClient starting request %d\n", numRequests);

        clientSocket = PR_NewTCPSocket();
        if (!clientSocket) {
            if (debug_mode) PR_fprintf(outFile, "Client error creating socket: OS error %d\n",
		    PR_GetOSError());
            continue;
        }

        if (debug_mode) DPRINTF(outFile, "\tClient connecting\n");

        rv = PR_Connect(clientSocket, 
                        &serverAddr,
                        PR_INTERVAL_NO_TIMEOUT);
        if (!clientSocket) {
            if (debug_mode) PR_fprintf(outFile, "\tClient error connecting\n");
            continue;
        }

        if (debug_mode) DPRINTF(outFile, "\tClient connected\n");
        ...
        rv = PR_Send(clientSocket, 
                     ...
                     PR_INTERVAL_NO_TIMEOUT);
        if (rv != _client_data) {
            if (debug_mode) PR_fprintf(outFile, "Client error sending data (%d)\n", rv);
            PR_Close(clientSocket);
            continue;
        }

        if (debug_mode) DPRINTF(outFile, "\tClient sent %d bytes\n", rv);
        ...
        while(bytesNeeded) {
            rv = PR_Recv(clientSocket, 
                         ...
                         PR_INTERVAL_NO_TIMEOUT);
            if (rv <= 0) {
                if (debug_mode) PR_fprintf(outFile, "Client error receiving data (%d) (%d/%d)\n", 
                    rv, (_server_data - bytesNeeded), _server_data);
                break;
            }
            if (debug_mode) DPRINTF(outFile, "\tClient received %d bytes; need %d more\n", rv, bytesNeeded - rv);
            bytesNeeded -= rv;
        }
...
RunClients(void)
{
    ...
    for (index=0; index<_clients; index++) {
        ...
        if (!clientThread) {
            ...
            if (debug_mode) DPRINTF(outFile, "\tMain created client %d/%d\n", index+1, _clients);
...
void do_work()
{
    ...
    ServerThread = PR_CreateThread(
                      ...
                      THREAD_STACKSIZE);
    if (!ServerThread) {
        if (debug_mode) PR_fprintf(outFile, "error creating main server thread\n");
        return;
    }
...
#endif
{
	...
	*/
    ...
    PR_STDIO_INIT();
    outFile = PR_GetSpecialFD(PR_StandardOutput);

 /* main test */
#if !defined(SYMBIAN) && !defined(NN_NINTENDO_SDK)
    if (debug_mode) {
		PR_fprintf(outFile, "Enter number of iterations: \n");
		scanf("%d", &_iterations);
		PR_fprintf(outFile, "Enter number of clients   : \n");
		scanf("%d", &_clients);
		PR_fprintf(outFile, "Enter size of client data : \n");
		scanf("%d", &_client_data);
		PR_fprintf(outFile, "Enter size of server data : \n");
    ...
    if (debug_mode) {
		...
		PR_fprintf(outFile, "Sending %d bytes of client data and %d bytes of server data\n", 
