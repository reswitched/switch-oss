...
#ifdef NN_NINTENDO_SDK
#if defined(WIN32)
    /*  Take the same as XP_PC below (use Windows calls)  */
...
#if defined(XP_PC) || (defined(NN_NINTENDO_SDK) && defined(WIN32))
...
#endif /* XP_PC */
#if defined(XP_UNIX) || defined(XP_BEOS) || (defined(NN_NINTENDO_SDK) && !defined(WIN32))
    if (strstr(lib, PR_DLL_SUFFIX) == NULL)
    {
        if (path) {
...
#ifdef NN_NINTENDO_SDK
    result = nnsdkLoadLibrary(name, flags, &oserr);
    goto unlock;
#else
