...
Ref<BidiContext> BidiContext::create(unsigned char level, UCharDirection direction, bool override, BidiEmbeddingSource source, BidiContext* parent)
{
    ...
    if (!level) {
        if (!override) {
            ...
            return ltrContext;
#else
            WKC_DEFINE_STATIC_TYPE(BidiContext*, ltrContext, &createUncached(0, U_LEFT_TO_RIGHT, false, FromStyleOrDOM, 0).leakRef());
            return *ltrContext;
#endif
        }
...
#else
        WKC_DEFINE_STATIC_TYPE(BidiContext*, ltrOverrideContext, &createUncached(0, U_LEFT_TO_RIGHT, true, FromStyleOrDOM, 0).leakRef());
        return *ltrOverrideContext;
#endif
    }
    ...
    if (!override) {
        ...
        return rtlContext;
#else
        WKC_DEFINE_STATIC_TYPE(BidiContext*, rtlContext, &createUncached(1, U_RIGHT_TO_LEFT, false, FromStyleOrDOM, 0).leakRef());
        return *rtlContext;
#endif
    }
...
#else
    WKC_DEFINE_STATIC_TYPE(BidiContext*, rtlOverrideContext, &createUncached(1, U_RIGHT_TO_LEFT, true, FromStyleOrDOM, 0).leakRef());
    return *rtlOverrideContext;
#endif
}
