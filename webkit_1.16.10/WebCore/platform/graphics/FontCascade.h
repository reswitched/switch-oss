...
#else
    static float getTextWidth(const FontPlatformData& in_font, int in_flags, const unsigned short* in_str, int in_len, float* out_clip_width);
    WKC_DEFINE_GLOBAL_CLASS_OBJ_ENTRY(CodePath, s_codePath);
    static UChar* fixedGlyphs(int sch, bool isemoji, const UChar* str, int& len, bool& needfree);
#endif
