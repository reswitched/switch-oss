...
static const StyleProperties& leftToRightDeclaration()
{
    ...
    return declaration;
#else
    WKC_DEFINE_STATIC_TYPE(MutableStyleProperties*, declaration, &MutableStyleProperties::create().leakRef());
    if (declaration->isEmpty())
        declaration->setProperty(CSSPropertyDirection, CSSValueLtr);
    return *declaration;
#endif
}
...
static const StyleProperties& rightToLeftDeclaration()
{
    ...
    WKC_DEFINE_STATIC_TYPE(MutableStyleProperties*, declaration, &MutableStyleProperties::create().leakRef());
    if (declaration->isEmpty())
        declaration->setProperty(CSSPropertyDirection, CSSValueRtl);
    return *declaration;
#endif
}
