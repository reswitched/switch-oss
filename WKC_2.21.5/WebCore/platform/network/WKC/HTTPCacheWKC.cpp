...
bool HTTPCachedResource::setResourceResponse(const ResourceResponse &response, Vector<std::pair<String, String>> varyingRequestHeaders, bool serverpush)
{
    ...
    m_lastModifiedHeader = response.httpHeaderField(HTTPHeaderName::LastModified);
    m_eTagHeader = response.httpHeaderField(HTTPHeaderName::ETag);
    m_age = response.age() ? (double)response.age().value().value() : numeric_limits<double>::quiet_NaN();
    m_accessControlAllowOriginHeader = response.httpHeaderField(HTTPHeaderName::AccessControlAllowOrigin);
