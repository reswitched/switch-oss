...
GraphicsLayerWKC::addChild(Ref<GraphicsLayer>&& childlayer)
{
    ...
    GraphicsLayer::addChild(childlayer.copyRef());
    if (changed)
        GraphicsLayerWKC_wkc(&childlayer.leakRef())->layerDidAttachToTree(m_wkc);
}
...
{
    bool changed = (childlayer->parent() != this);
    GraphicsLayer::addChildAtIndex(childlayer.copyRef(), index);
    if (changed)
        GraphicsLayerWKC_wkc(&childlayer.leakRef())->layerDidAttachToTree(m_wkc);
}
...
{
    bool changed = (childlayer->parent() != this);
    GraphicsLayer::addChildAbove(childlayer.copyRef(), sibling);
    if (changed)
        GraphicsLayerWKC_wkc(&childlayer.leakRef())->layerDidAttachToTree(m_wkc);
}
...
{
    bool changed = (childlayer->parent() != this);
    GraphicsLayer::addChildBelow(childlayer.copyRef(), sibling);
    if (changed)
        GraphicsLayerWKC_wkc(&childlayer.leakRef())->layerDidAttachToTree(m_wkc);
}
...
GraphicsLayerWKC::replaceChild(GraphicsLayer* oldChild, Ref<GraphicsLayer>&& newChild)
{
    ...
    bool replaced = GraphicsLayer::replaceChild(oldChild, newChild.copyRef());
    if (replaced) {
        GraphicsLayerWKC_wkc(oldChild)->layerDidDetachFromTree();
        if (changed)
            GraphicsLayerWKC_wkc(&newChild.leakRef())->layerDidAttachToTree(m_wkc);
    }
    return replaced;
}
