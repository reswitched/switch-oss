...
{
    m_appClient->setToolTip(string, toWKCTextDirection(dir));
}
...
void
ChromeClientWKC::attachViewOverlayGraphicsLayer(WebCore::Frame* frame, WebCore::GraphicsLayer* layer)
{
    WKC::FramePrivate wf(frame);
    if (layer) {
        // attach
        WKC::GraphicsLayerPrivate* wg = GraphicsLayerWKC_wkc(layer);
        m_appClient->attachViewOverlayGraphicsLayer(&wf.wkc(), &wg->wkc());
    } else {
        // detach
        m_appClient->attachViewOverlayGraphicsLayer(&wf.wkc(), 0);
    }
}
...
void
ChromeClientWKC::didAssociateFormControls(const WTF::Vector<WTF::RefPtr<WebCore::Element> >&)
{
    notImplemented();
}
