...
{
    registrar(create, getSupportedTypes, supportsType, getSitesInMediaCache, clearMediaCache, clearMediaCacheForSite, supportsKeySystem);
}
...
{
    const char* keySystem = 0;
#if ENABLE(ENCRYPTED_MEDIA)
    keySystem = parameters.keySystem.utf8().data();
#endif
    int ret = wkcMediaPlayerIsSupportedMIMETypePeer(parameters.type.utf8().data(), parameters.codecs.utf8().data(), keySystem, parameters.url.string().utf8().data());
    switch (ret) {
...
MediaPlayerPrivate::paint(GraphicsContext* ctx, const FloatRect& r)
{
    ...
    case WKC_MEDIA_VIDEOSINKTYPE_BITMAP:
    {
        ...
        if ((fmt&WKC_IMAGETYPE_TYPEMASK) == WKC_IMAGETYPE_ARGB8888) {
            ...
            ImageContainerWKC wimg(ImageWKC::EColorARGB8888, bitmap, rowbytes, size);
    ...
    {
        ctx->save();
        ctx->fillRect(r, Color::black, ColorSpaceDeviceRGB);
        ctx->restore();
        break;
    }
...
MediaPlayerPrivate::notifyPlayerStreamOpenURI(const char* uri, void* procs)
{
    ...
    ResourceHandle* handle = WebCore::ResourceHandle::create(0, req, this, false, false).leakRef();
