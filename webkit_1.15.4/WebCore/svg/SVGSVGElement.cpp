...
#if PLATFORM(WKC)
    if (defaultScriptType.isNull())
        defaultScriptType.construct(AtomicString { "text/ecmascript" });
#endif
...
#if PLATFORM(WKC)
    if (defaultStyleType.isNull())
        defaultStyleType.construct(AtomicString { "text/css" });
#endif
