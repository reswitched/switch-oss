...
{
#if PLATFORM(WKC)
    WKC_DEFINE_STATIC_HASHMAP_PTR(const char*, GCTypeMap<SimpleStats>, result, nullptr);
    if (!result) {
...
#if PLATFORM(WKC)
                slotVisitor->drainFromShared(SlotVisitor::SlaveDrain, MonotonicTime::fromRawSeconds(20.));
#else
