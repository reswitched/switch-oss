...
#if PLATFORM(WKC)
#include <wtf/icu/unicode/UTF8.h>
#else
...
#else
    WKC_DEFINE_STATIC_TYPE(Bitmap<256>*, doNotUnescapeWhenDecodingURI, new Bitmap<256>);
    *doNotUnescapeWhenDecodingURI = makeCharacterBitmap(
        "#$&+,/:;=?@"
    );
...
#else
    WKC_DEFINE_STATIC_TYPE(Bitmap<256>*, emptyBitmap, new Bitmap<256>);
    return JSValue::encode(decode(exec, *emptyBitmap, true));
#endif
}
...
#else
    WKC_DEFINE_STATIC_TYPE(Bitmap<256>*, doNotEscapeWhenEncodingURI, new Bitmap<256>);
    *doNotEscapeWhenEncodingURI = makeCharacterBitmap(
        "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        "abcdefghijklmnopqrstuvwxyz"
        "0123456789"
        "!#$&'()*+,-./:;=?@_~"
...
#else
    WKC_DEFINE_STATIC_TYPE(Bitmap<256>*, doNotEscapeWhenEncodingURIComponent, new Bitmap<256>);
    *doNotEscapeWhenEncodingURIComponent = makeCharacterBitmap(
        "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        "abcdefghijklmnopqrstuvwxyz"
        "0123456789"
        "!'()*-._~"
...
#else
    WKC_DEFINE_STATIC_TYPE(Bitmap<256>*, doNotEscape_, new Bitmap<256>);
    Bitmap<256>& doNotEscape = *doNotEscape_;
    doNotEscape = makeCharacterBitmap(
        "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        "abcdefghijklmnopqrstuvwxyz"
        "0123456789"
        "*+-./@_"
