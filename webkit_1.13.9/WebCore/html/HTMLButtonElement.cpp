...
#if PLATFORM(WKC)
            if (submit.isNull())
                submit.construct("submit", AtomicString::ConstructFromLiteral);
#endif
            return submit;
        }
        case BUTTON: {
...
#if PLATFORM(WKC)
            if (button.isNull())
                button.construct("button", AtomicString::ConstructFromLiteral);
#endif
            return button;
        }
        case RESET: {
...
#if PLATFORM(WKC)
            if (reset.isNull())
                reset.construct("reset", AtomicString::ConstructFromLiteral);
#endif
            return reset;
        }
    }
