...
#if PLATFORM(WKC)
    if (mimeTypeToLabelTitleMap.isNull())
        mimeTypeToLabelTitleMap.construct();
#endif
...
#if PLATFORM(WKC)
    if (mimeTypeToLabelSubtitleMap.isNull())
        mimeTypeToLabelSubtitleMap.construct();
#endif
...
#else
    WKC_DEFINE_STATIC_TYPE(DOMWrapperWorld*, isolatedWorld, &DOMWrapperWorld::create(commonVM()).leakRef());
    return *isolatedWorld;
#endif
}
...
#if PLATFORM(WKC)
    if (selector.isNull())
        selector.construct(".snapshot-overlay", AtomicString::ConstructFromLiteral);
#endif
