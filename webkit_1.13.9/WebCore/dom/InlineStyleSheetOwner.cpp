...
#if PLATFORM(WKC)
    auto& baseURL = shadowRoot && shadowRoot->mode() == ShadowRootMode::UserAgent ? blankURL() : element.document().baseURL().isNull() ? blankURL() : element.document().baseURL();
#else
...
inline bool isValidCSSContentType(Element& element, const AtomicString& type)
{
    ...
    if (cssContentType.isNull())
        cssContentType.construct("text/css", AtomicString::ConstructFromLiteral);
#endif
