...
#if PLATFORM(WKC)
    size_t bytesToBeAllocated = m_data->needsReallocationToMergeBuffers();
...
#if PLATFORM(WKC)
    clearBeforeFrame = std::min(clearBeforeFrame, static_cast<size_t>(m_currentFrame));
    if (clearBeforeFrame) {
        const auto* prevBuffer = &m_frameBufferCache[--clearBeforeFrame];
        auto prevMethod = prevBuffer->disposalMethod();
        while (clearBeforeFrame && (prevMethod == ScalableImageDecoderFrame::DisposalMethod::RestoreToPrevious)) {
            prevBuffer = &m_frameBufferCache[--clearBeforeFrame];
