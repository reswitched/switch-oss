...
#else
    WKC_DEFINE_STATIC_TYPE(const SVGPropertyInfo*, s_propertyInfo, 0);
#endif
    if (!s_propertyInfo) {
...
#if PLATFORM(WKC)
    if (s_identifier.isNull())
        s_identifier.construct("SVGOrientType", AtomicString::ConstructFromLiteral);
#endif
    return s_identifier;
}
...
#if PLATFORM(WKC)
    if (s_identifier.isNull())
        s_identifier.construct("SVGOrientAngle", AtomicString::ConstructFromLiteral);
#endif
    return s_identifier;
}
...
#if PLATFORM(WKC)
    if (autoString.isNull())
        autoString.construct("auto", AtomicString::ConstructFromLiteral);
#endif
    static NeverDestroyed<AtomicString> autoStartReverseString("auto-start-reverse", AtomicString::ConstructFromLiteral);
#if PLATFORM(WKC)
    if (autoStartReverseString.isNull())
        autoStartReverseString.construct("auto-start-reverse", AtomicString::ConstructFromLiteral);
#endif
