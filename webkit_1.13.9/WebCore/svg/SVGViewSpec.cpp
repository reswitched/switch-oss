...
#else
    WKC_DEFINE_STATIC_TYPE(const SVGPropertyInfo*, s_propertyInfo, nullptr)
#endif
    if (!s_propertyInfo) {
...
const AtomicString& SVGViewSpec::viewBoxIdentifier()
{
    ...
    if (s_identifier.isNull())
        s_identifier.construct("SVGViewSpecViewBoxAttribute", AtomicString::ConstructFromLiteral);
#endif
    return s_identifier;
}
...
#if PLATFORM(WKC)
    if (s_identifier.isNull())
        s_identifier.construct("SVGViewSpecPreserveAspectRatioAttribute", AtomicString::ConstructFromLiteral);
#endif
    return s_identifier;
}
...
#if PLATFORM(WKC)
    if (s_identifier.isNull())
        s_identifier.construct("SVGViewSpecTransformAttribute", AtomicString::ConstructFromLiteral);
#endif
    return s_identifier;
}
