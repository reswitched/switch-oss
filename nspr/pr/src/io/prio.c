...
static PRStatus PR_CALLBACK nnsdkStdIoClose(PRFileDesc *fd) {
...
static PRStatus PR_CALLBACK nnsdkStdIoFsync(PRFileDesc *fd) {
    /*  Nothing to do, just return success  */
    return PR_SUCCESS;
}

/*  Create a special layered I/O piece to handle std in, out and err.
    Std in will just return EOF.  Std out and error will use NNSDK's
    logging to provide output.  */
static const struct PRIOMethods NNSDK_STD_OUTERR_METHODS = {
    PR_DESC_LAYERED,
    nnsdkStdIoClose,
...
static const PRIOMethods *_nnsdkGetStdOutErrMethods() {
    return &NNSDK_STD_OUTERR_METHODS;
}

static const PRIOMethods *_nnsdkGetStdInMethods() {
    return &_pr_faulty_methods;
}

static PRFileDesc *_nnsdkGetStdInOutErr(int stdfd) {
    PRFileDesc                  *fd;
    ...
    if (fd == NULL) {
        ...
        switch (stdfd) {
            case 0:
                fd->methods = _nnsdkGetStdInMethods();
                break;

            case 1:
            case 2:
            default:
                fd->methods = _nnsdkGetStdOutErrMethods();
                break;
        }
    }
...
#if defined(NN_NINTENDO_SDK)
    _pr_stdin = _nnsdkGetStdInOutErr(0);
    _pr_stdout = _nnsdkGetStdInOutErr(1);
    _pr_stderr = _nnsdkGetStdInOutErr(2);
#elif defined(WIN32)
