...
PRThread dummyThreadObj;

/*  Special case handling of attached threads in NNSDK environment since there is no
    signal handler or other mechanism to "hook" in the thread's context
    which can be used to clean it up.  */
void _PRI_DetachOtherThread(PRThread *thread) {
    _PR_CleanupThread(thread);
    PR_DELETE(thread->privateData);

    /*  Decrementing the active count is ugly because NSPR expects
        that the thread calling this is something known to NSPR but
        we're using this to detach something which is gone.  So the
        usual internal calls of lock and cv set won't work, we do the
        same logic here, which is not ideal.
        THIS ASSUMES THE NNSDK, GLOBAL THREAD ONLY MODEL.  */
    _PR_MD_LOCK(&_pr_activeLock->ilock);
    _pr_activeLock->owner = &dummyThreadObj;
    if (thread->flags & _PR_SYSTEM) {
        _pr_systemActive--;
    } else {
        _pr_userActive--;
        if (_pr_userActive == _pr_primordialExitCount) {
            _PR_MD_NOTIFY_CV(&_pr_primordialExitCVar->md,
                             &_pr_primordialExitCVar->lock->ilock);
        }
    }

    _pr_activeLock->owner = 0;
    _PR_MD_UNLOCK(&_pr_activeLock->ilock);

    _PR_MD_CLEAN_THREAD(thread);
    if (!thread->threadAllocatedOnStack) {
        PR_DELETE(thread->stack);
    }
