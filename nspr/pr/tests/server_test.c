...
SetServerState(char *waiter, PRInt32 state)
{
    ...
    if (debug_mode) {
        DPRINTF(outFile, "\t%s changed state to %d\n", waiter, state);
    }
...
WaitServerState(char *waiter, PRInt32 state)
{
    ...
    if (debug_mode) {
        DPRINTF(outFile, "\t%s waiting for state %d\n", waiter, state);
    }
...
WorkerThreadFunc(void *_listenSock)
{
    ...
    if (!dataBuf)
        if (debug_mode) {
            PR_fprintf(outFile, "\tServer could not malloc space!?\n");
        }
    ...
    if (debug_mode) {
        DPRINTF(outFile, "\tServer worker thread running\n");
    }
    ...
    while(1) {
        ...
        if (debug_mode) {
            DPRINTF(outFile, "\tServer thread going into accept\n");
        }
        ...
        if (bytesRead < 0) {
            if (debug_mode) {
                PR_fprintf(outFile, "\tServer error in accept (%d)\n", bytesRead);
            }
            continue;
        }

        if (debug_mode) {
            DPRINTF(outFile, "\tServer accepted connection (%d bytes)\n", bytesRead);
        }
        ...
        if (workerThreadsBusy == workerThreads) {
            ...
            if (workerThreadsBusy == workerThreads) {
                ...
                if (!WorkerThread) {
                    if (debug_mode) {
                        PR_fprintf(outFile, "Error creating client thread %d\n", workerThreads);
                    }
                } else {
                    PR_AtomicIncrement(&workerThreads);
                    if (debug_mode) {
                        DPRINTF(outFile, "\tServer creates worker (%d)\n", workerThreads);
                    }
                }
            }
        ...
        while (bytesToRead) {
            ...
            if (bytesRead < 0) {
                if (debug_mode) {
                    PR_fprintf(outFile, "\tServer error receiving data (%d)\n", bytesRead);
                }
                continue;
            }
            if (debug_mode) {
                DPRINTF(outFile, "\tServer received %d bytes\n", bytesRead);
            }
        }
        ...
        if (bytesWritten != _server_data) {
            ...
            if (debug_mode) {
                DPRINTF(outFile, "\tServer sent %d bytes\n", bytesWritten);
            }
        }
...
ServerSetup(void)
{
    ...
    if ((listenSocket = PR_NewTCPSocket()) == NULL) {
        if (debug_mode) {
            PR_fprintf(outFile, "\tServer error creating listen socket\n");
        }
        else {
            Test_Result(FAIL);
        }
    ...
    if (PR_Listen(listenSocket, 128) != PR_SUCCESS) {
        if (debug_mode) {
            PR_fprintf(outFile, "\tServer error listening to server socket\n");
        }
        else {
            Test_Result(FAIL);
        }
    ...
    if (!WorkerThread) {
        if (debug_mode) {
            PR_fprintf(outFile, "error creating working thread\n");
        }
        PR_Close(listenSocket);
        return NULL;
    }
    PR_AtomicIncrement(&workerThreads);
    if (debug_mode) {
        DPRINTF(outFile, "\tServer created primordial worker thread\n");
    }
...
ClientThreadFunc(void *unused)
{
    ...
    if (!sendBuf)
        if (debug_mode) {
            PR_fprintf(outFile, "\tClient could not malloc space!?\n");
        }
    ...
    while(numRequests > 0) {
        ...
        if (debug_mode) {
            DPRINTF(outFile, "\tClient starting request %d\n", numRequests);
        }
        ...
        if (debug_mode) {
            DPRINTF(outFile, "\tClient connecting\n");
        }
        ...
        if (!clientSocket) {
            if (debug_mode) {
                PR_fprintf(outFile, "\tClient error connecting\n");
            }
            continue;
        }

        if (debug_mode) {
            DPRINTF(outFile, "\tClient connected\n");
        }
        ...
        if (rv != _client_data) {
            if (debug_mode) {
                PR_fprintf(outFile, "Client error sending data (%d)\n", rv);
            }
        ...
        if (debug_mode) {
            DPRINTF(outFile, "\tClient sent %d bytes\n", rv);
        }
        ...
        while(bytesNeeded) {
            ...
            if (debug_mode) {
                DPRINTF(outFile, "\tClient received %d bytes; need %d more\n", rv, bytesNeeded - rv);
            }
            bytesNeeded -= rv;
        }
...
RunClients(void)
{
    ...
    for (index=0; index<_clients; index++) {
        ...
        if (!clientThread) {
            ...
            DPRINTF(outFile, "\tMain created client %d/%d\n", index+1, _clients);
        }
...
void do_work()
{
    ...
    if (!ServerThread) {
        if (debug_mode) {
            PR_fprintf(outFile, "error creating main server thread\n");
        }
        return;
    }
