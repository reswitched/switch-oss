...
static PRThread* g_pWorkerThreads[MAX_WORKERS] = {NULL};
...
static PRStatus NewThread(
    StartFn start, void *arg, PRThreadPriority prio, PRThreadState state, int isWorker)
#else
static PRStatus NewThread(
...
#endif
{
    ...
    switch (thread_provider)
    {
        case thread_nspr:
        {
            ...
            if(isWorker)
            {
                TEST_ASSERT(g_workerCount < MAX_WORKERS);

                g_pWorkerThreads[g_workerCount] = thread;
                if(g_pWorkerThreads[g_workerCount])
                {
                    ++g_workerCount;
                }
            }
...
#ifdef NN_NINTENDO_SDK
    rv = NewThread(
             Worker, worker, DEFAULT_SERVER_PRIORITY, PR_UNJOINABLE_THREAD, 1);
#else
    rv = NewThread(
...
#ifdef NN_NINTENDO_SDK
        rv = NewThread(
                 Server, server, PR_PRIORITY_HIGH, PR_JOINABLE_THREAD, 0);
#else
        rv = NewThread(
...
#ifdef NN_NINTENDO_SDK
            rv = NewThread(
                     Client, &client[index], PR_PRIORITY_NORMAL, PR_JOINABLE_THREAD, 0);
#else
            rv = NewThread(
...
#ifdef NN_NINTENDO_SDK
    for(unsigned int iThread = 0; iThread < g_workerCount; ++iThread)
    {
        PR_JoinThread(g_pWorkerThreads[iThread]);
        g_pWorkerThreads[iThread] = NULL;
    }
    g_workerCount = 0;
#endif
