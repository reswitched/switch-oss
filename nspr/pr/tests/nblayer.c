...
static void PR_CALLBACK Client(void *arg)
{
    ...
    if ((PR_FAILURE == rv) && (PR_IN_PROGRESS_ERROR == PR_GetError()))
    {
        ...
        do
        {
            ...
            if (verbosity >= chatty) {
                PR_fprintf(logFile, "Client conn/poll ready %d, in_flags %x, out_flags %x\n", ready, polldesc.in_flags, polldesc.out_flags);
            }
    ...
    for (mits = 0; mits < minor_iterations; ++mits)
    {
        ...
        do
        {
            ...
            else if ((-1 == ready) && (PR_WOULD_BLOCK_ERROR == PR_GetError()))
            {
                ...
                ready = PR_Poll(&polldesc, 1, PR_INTERVAL_NO_TIMEOUT);
                if (verbosity >= chatty) {
                    PR_fprintf(logFile, "Client send/poll ready %d, in_flags %x, out_flags %x\n", ready, polldesc.in_flags, polldesc.out_flags);
                }
        ...
        do
        {
            ...
            else if ((-1 == ready) && (PR_WOULD_BLOCK_ERROR == PR_GetError()))
            {
                ...
                ready = PR_Poll(&polldesc, 1, PR_INTERVAL_NO_TIMEOUT);
                if (verbosity >= chatty) {
                    PR_fprintf(logFile, "Client rcv/poll ready %d, in_flags %x, out_flags %x\n", ready, polldesc.in_flags, polldesc.out_flags);
                }
...
static void PR_CALLBACK Server(void *arg)
{
    ...
    do
    {
        ...
        if ((NULL == service) && (PR_WOULD_BLOCK_ERROR == PR_GetError()))
        {
            ...
            ready = PR_Poll(&polldesc, 1, PR_INTERVAL_NO_TIMEOUT);
            if (verbosity >= chatty) {
                PR_fprintf(logFile, "Server accept/poll ready %d, in_flags %x, out_flags %x\n", ready, polldesc.in_flags, polldesc.out_flags);
            }
    ...
    do
    {
        ...
        do
        {
            ...
            else if ((-1 == ready) && (PR_WOULD_BLOCK_ERROR == PR_GetError()))
            {
                ...
                ready = PR_Poll(&polldesc, 1, PR_INTERVAL_NO_TIMEOUT);
                if (verbosity >= chatty) {
                    PR_fprintf(logFile, "Server rcv/poll ready %d, in_flags %x, out_flags %x\n", ready, polldesc.in_flags, polldesc.out_flags);
                }
        ...
        if (0 != bytes_read)
        {
            ...
            do
            {
                ...
                else if ((-1 == ready) && (PR_WOULD_BLOCK_ERROR == PR_GetError()))
                {
                    ...
                    ready = PR_Poll(&polldesc, 1, PR_INTERVAL_NO_TIMEOUT);
                    if (verbosity >= chatty) {
                        PR_fprintf(logFile, "Server send/poll ready %d, in_flags %x, out_flags %x\n", ready, polldesc.in_flags, polldesc.out_flags);
                    }
