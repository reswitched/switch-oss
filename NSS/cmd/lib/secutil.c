...
SECU_GetPasswordString(void *arg, char *prompt)
{
    ...
    PR_fprintf(PR_STDERR, "[ERR] SECU_GetPasswordString: not supported\n");
    return NULL;
#else
#ifndef _WINDOWS
    char *p = NULL;
...
SECU_FilePasswd(PK11SlotInfo *slot, PRBool retry, void *arg)
{
    ...
    if (!fd) {
        PR_fprintf(PR_STDERR, "No password file \"%s\" exists.\n", pwFile);
        PORT_Free(phrases);
        return NULL;
    }
                       ...
                       struct SECKEYPrivateKeyStr **pRetKey)
{
    ...
    if (arg == NULL) {
        PR_fprintf(PR_STDERR, "no key/cert name specified for client auth\n");
        return -1;
    }
    ...
    if (!cert) {
        if (errsave == SEC_ERROR_BAD_PASSWORD)
            PR_fprintf(PR_STDERR, "Bad password\n");
        else if (errsave > 0)
            PR_fprintf(PR_STDERR, "Unable to read cert (error %d)\n", errsave);
        else if (errsave == SEC_ERROR_BAD_DATABASE)
            PR_fprintf(PR_STDERR, "Unable to get cert from database (%d)\n", errsave);
        else
            PR_fprintf(PR_STDERR, "SECKEY_FindKeyByName: internal error %d\n", errsave);
        return -1;
    }

    key = PK11_FindKeyByAnyCert(arg, NULL);
    if (!key) {
        PR_fprintf(PR_STDERR, "Unable to get key (%d)\n", PORT_GetError());
        return -1;
    }
                     ...
                     PRBool warnOnPrivateKeyInAsciiFile)
{
    ...
    if (ascii) {
        ...
        if (warnOnPrivateKeyInAsciiFile && strstr(asc, "PRIVATE KEY")) {
            PR_fprintf(PR_STDERR, "Warning: ignoring private key. Consider to use "
                            "pk12util.\n");
        }
        ...
        if (rv != SECSuccess) {
            PR_fprintf(PR_STDERR, "error converting der (%s)\n",
                    SECU_Strerror(PORT_GetError()));
            return SECFailure;
        }
    }
