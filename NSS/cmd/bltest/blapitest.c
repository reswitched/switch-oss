...
#ifdef NN_NINTENDO_SDK
    PRINTUSAGE("",       "-6", "Set the nnsdk host mount location");
#endif
...
getECParams(const char *curve)
{
    ...
    if ((curveOidTag == SEC_OID_UNKNOWN) ||
        ...
        PR_fprintf(PR_STDERR, "Unrecognized elliptic curve %s\n", curve);
        return NULL;
    }
...
print_exponent(SECItem *exp)
{
    ...
    if (exp->len <= 4) {
        ...
        PR_fprintf(PR_STDOUT, "%12d", e);
    } else {
        e = 8 * exp->len;
        PR_fprintf(PR_STDOUT, "~2**%-8d", e);
    }
}
...
{
    if (reportStr) {
        PR_fprintf(PR_STDOUT, sformat, reportStr);
        PR_smprintf_free(reportStr);
    } else {
        PR_fprintf(PR_STDOUT, nformat, rNum);
    }
}
                      ...
                      PRBool encrypt, PRBool cxonly)
{
    ...
    PR_fprintf(PR_STDOUT, "#%9s", "mode");
    PR_fprintf(PR_STDOUT, "%12s", "in");
print_td:
    switch (info->mode) {
        case bltestDES_ECB:
        case bltestDES_CBC:
        ...
        case bltestRC4:
            if (td)
                PR_fprintf(PR_STDOUT, "%8s", "symmkey");
            else
                PR_fprintf(PR_STDOUT, "%8d", 8 * info->params.sk.key.buf.len);
            break;
#ifdef NSS_SOFTOKEN_DOES_RC5
        case bltestRC5_ECB:
        case bltestRC5_CBC:
        ...
        case bltestRSA_OAEP:
            if (td) {
                PR_fprintf(PR_STDOUT, "%8s", "rsa_mod");
                PR_fprintf(PR_STDOUT, "%12s", "rsa_pe");
            } else {
                bltestAsymKeyParams *asymk = &info->params.asymk;
                PR_fprintf(PR_STDOUT, "%8d", asymk->cipherParams.rsa.keysizeInBits);
                print_exponent(
        ...
        case bltestDSA:
            if (td) {
                PR_fprintf(PR_STDOUT, "%8s", "pqg_mod");
            } else {
                PR_fprintf(PR_STDOUT, "%8d", info->params.asymk.cipherParams.dsa.keysize);
            }
            break;
#ifndef NSS_DISABLE_ECC
        case bltestECDSA:
            if (td) {
                PR_fprintf(PR_STDOUT, "%12s", "ec_curve");
            } else {
                ECPrivateKey *key = (ECPrivateKey *)info->params.asymk.privKey;
                ECCurveName curveName = key->ecParams.name;
                PR_fprintf(PR_STDOUT, "%12s",
    ...
    if (!td) {
        ...
        PR_fprintf(PR_STDOUT, "%12.3f", cxtime);
        PR_fprintf(PR_STDOUT, "%12.3f", optime);
        PR_fprintf(PR_STDOUT, "%12.03f", totalTimeInt / 1000);
    ...
    PR_fprintf(PR_STDOUT, "%8s", "opreps");
    PR_fprintf(PR_STDOUT, "%8s", "cxreps");
    PR_fprintf(PR_STDOUT, "%12s", "context");
    PR_fprintf(PR_STDOUT, "%12s", "op");
    PR_fprintf(PR_STDOUT, "%12s", "time(sec)");
    PR_fprintf(PR_STDOUT, "%12s", "thrgput");
    PR_fprintf(PR_STDOUT, "\n");
    PR_fprintf(PR_STDOUT, "%8s", mode_strings[info->mode]);
    PR_fprintf(PR_STDOUT, "_%c", (cxonly) ? 'c' : (encrypt) ? 'e' : 'd');
...
printmodes()
{
    ...
    PR_fprintf(PR_STDERR, "%s: Available modes (specify with -m):\n", progName);
...
static void cleanupTempParamsData(bltestCipherMode mode, bltestParams *params)
{
	switch (mode) {
		case bltestRSA_PSS:
		case bltestRSA_OAEP:
		case bltestRSA:
			if (params->asymk.privKey != NULL) {
				RSAPrivateKey *key = (RSAPrivateKey	*)params->asymk.privKey;
				if (key->arena != NULL)	{
					PORT_FreeArena(key->arena, PR_TRUE);
					key->arena = NULL;
				}

				params->asymk.privKey =	NULL;
			}
		...
		case bltestDSA:
			if (params->asymk.privKey != NULL) {
				DSAPrivateKey *key = (DSAPrivateKey	*)params->asymk.privKey;
				if (key->params.arena != NULL) {
					PORT_FreeArena(key->params.arena, PR_TRUE);
					key->params.arena =	NULL;
				}

				params->asymk.privKey =	NULL;
			}

			if (params->asymk.cipherParams.dsa.pqg != NULL)	{
				PQGParams *pqg = (PQGParams	*)params->asymk.cipherParams.dsa.pqg;
				if (pqg->arena != NULL)	{
					PORT_FreeArena(pqg->arena, PR_TRUE);
					pqg->arena = NULL;
				}

				params->asymk.cipherParams.dsa.pqg = NULL;
			}
		...
		case bltestECDSA:
			if (params->asymk.privKey != NULL) {
				ECPrivateKey *key =	(ECPrivateKey *)params->asymk.privKey;
				if (key->ecParams.arena	!= NULL) {
					PORT_FreeArena(key->ecParams.arena,	PR_TRUE);
					key->ecParams.arena	= NULL;
				}
               ...
               PRBool encrypt, PRBool decrypt)
{
    ...
    for (i = 0; i < nummodes; i++) {
        ...
        if (mode == bltestINVALID) {
            PR_fprintf(PR_STDERR, "%s: Skipping invalid mode.\n", progName);
            continue;
        }
        ...
        if (ReadFileToItem(arena, &item, filename) != SECSuccess) {
            PR_fprintf(PR_STDERR, "%s: Cannot read file %s.\n", progName, filename);
            rv = SECFailure;
            continue;
        }
...
#define RSA_TEST_EQUAL(comp)                                   \
    if (!SECITEM_ItemsAreEqual(&(src->comp), &(dest->comp))) { \
        PR_fprintf(PR_STDERR, "key->" #comp " not equal");           \
        if (src->comp.len != dest->comp.len) {                 \
            PR_fprintf(PR_STDERR, "src_len = %d, dest_len = %d",     \
...
rsaPrivKeysAreEqual(RSAPrivateKey *src, RSAPrivateKey *dest)
{
    ...
    if (!areEqual) {
        PR_fprintf(PR_STDERR, "original key:\n");
        dump_rsakey(src);
        PR_fprintf(PR_STDERR, "recreated key:\n");
        dump_rsakey(dest);
    }
    return areEqual;
}
...
doRSAPopulateTest(unsigned int keySize, unsigned long exponent)
{
    ...
    if (srcKey == NULL) {
        PR_fprintf(PR_STDERR, "RSA Key Gen failed");
        return -1;
    }
    ...
    if (rv != SECSuccess) {
        PR_fprintf(PR_STDERR, "RSA Populate failed: pubExp mod p\n");
        failed = 1;
    } else if (!rsaPrivKeysAreEqual(&tstKey, srcKey)) {
        PR_fprintf(PR_STDERR, "RSA Populate key mismatch: pubExp mod p\n");
        failed = 1;
    }
    ...
    rv = RSA_PopulatePrivateKey(&tstKey);
    if (rv != SECSuccess) {
        PR_fprintf(PR_STDERR, "RSA Populate failed: pubExp mod q\n");
        failed = 1;
    } else if (!rsaPrivKeysAreEqual(&tstKey, srcKey)) {
        PR_fprintf(PR_STDERR, "RSA Populate key mismatch: pubExp mod q\n");
        failed = 1;
    }
    ...
    rv = RSA_PopulatePrivateKey(&tstKey);
    if (rv != SECSuccess) {
        PR_fprintf(PR_STDERR, "RSA Populate failed: privExp p q\n");
        failed = 1;
    } else if (!rsaPrivKeysAreEqual(&tstKey, srcKey)) {
        PR_fprintf(PR_STDERR, "RSA Populate key mismatch: privExp  p q\n");
        failed = 1;
    }
    ...
    rv = RSA_PopulatePrivateKey(&tstKey);
    if (rv != SECSuccess) {
        PR_fprintf(PR_STDERR, "RSA Populate failed: pubExp privExp q\n");
        PR_fprintf(PR_STDERR, " - not fatal\n");
     ...
     * than return the wrong key. */
        ...
        PR_fprintf(PR_STDERR, "RSA Populate key mismatch: pubExp privExp  q\n");
    ...
    rv = RSA_PopulatePrivateKey(&tstKey);
    if (rv != SECSuccess) {
        PR_fprintf(PR_STDERR, "RSA Populate failed: pubExp privExp mod\n");
        failed = 1;
    } else if (!rsaPrivKeysAreEqual(&tstKey, srcKey)) {
        PR_fprintf(PR_STDERR, "RSA Populate key mismatch: pubExp privExp  mod\n");
        failed = 1;
    }

    /* Reset the key when done so any internal arena is released  */
    rsaPrivKeyReset(&tstKey);
...
#endif
{
...
#if defined(NN_NINTENDO_SDK) && defined(NNSDK_NSS_TEST)
    /*  Reset the commands and options so the test can be run repeatedly  */
    for (i = 0; i < bltest.numCommands; i++) {
        bltest_commands[i].activated = 0;
    }

    for (i = 0; i < bltest.numOptions; i++) {
        bltest_options[i].activated = 0;
    }
#endif
    ...
    if (rv == SECFailure) {
        PR_fprintf(PR_STDERR, "%s: command line parsing error!\n", progName);
        goto print_usage;
    }
    rv = SECFailure;
    ...
    if (commandsEntered > 1 &&
        ...
        PR_fprintf(PR_STDERR, "%s: one command at a time!\n", progName);
        goto print_usage;
    }
...
#if defined(NN_NINTENDO_SDK) && !defined(NNSDK_NSS_TEST)
    if (bltest.options[opt_NnSdkHost].activated) {
        char                    *hostPath = bltest.options[opt_NnSdkHost].arg;

        if (hostPath == NULL) {
            PR_fprintf(PR_STDERR,
                    "%s: a valid host path must be provided to mount. Got \'%s\'\n",
                    progName,
                    hostPath);
            goto print_usage;
        }

        nnsdkSetupHostSystemFs(hostPath);
    }
#endif
    ...
    if (bltest.commands[cmd_RSAPopulate].activated) {
        ...
        if (ret != 0) {
            PR_fprintf(PR_STDERR, "RSA Populate test round %d: FAILED\n", i);
        }
        return ret;
    }
    ...
    if ((bltest.commands[cmd_Decrypt].activated ||
        ...
        PR_fprintf(PR_STDERR, "%s: Cannot use a nonce as input to decrypt/verify.\n",
                progName);
        goto print_usage;
    }
    ...
    if (bltest.options[opt_Mode].activated) {
        ...
        PR_fprintf(PR_STDERR, "%s: You must specify a cipher mode with -m.\n",
                progName);
        goto print_usage;
    }
    ...
    if (bltest.options[opt_Repetitions].activated &&
        ...
        PR_fprintf(PR_STDERR, "%s: Operation time should be defined in either "
    ...
    if (bltest.commands[cmd_Verify].activated &&
        ...
        PR_fprintf(PR_STDERR, "%s: You must specify a signature file with -f.\n",
                progName);
    ...
    for (curThrdNum = 0; curThrdNum < threads; curThrdNum++) {
        ...
        if (curThrdNum > 0) {
            ...
            if (!newCInfo) {
                PR_fprintf(PR_STDERR, "%s: Can not allocate  memory.\n", progName);
                goto exit_point;
            }
        ...
        if (!infile) {
            PR_fprintf(PR_STDERR, "%s: Failed to open input file.\n", progName);
            goto exit_point;
        }
        ...
        if (!outfile) {
            PR_fprintf(PR_STDERR, "%s: Failed to open output file.\n", progName);
            rv = SECFailure;
            goto exit_point;
        }
...
exit_point:
    ...
    while (cipherInfo != NULL) {
        ...
        if (cipherInfo->arena) {
            RSAPrivateKey *rsaKey = (RSAPrivateKey *)cipherInfo->params.asymk.privKey;
            bltestDSAParams *dsap = &cipherInfo->params.asymk.cipherParams.dsa;
            DSAPrivateKey *dsaKey = (DSAPrivateKey *)cipherInfo->params.asymk.privKey;
            ECPrivateKey *ecKey = (ECPrivateKey *)cipherInfo->params.asymk.privKey;

            switch (cipherInfo->mode) {
                case bltestRSA:
                case bltestRSA_PSS:
                case bltestRSA_OAEP:
                    if (rsaKey->arena != NULL) {
                ...
                case bltestDSA:
                    if ((dsap->pqg != NULL) && (dsap->pqg->arena != NULL)) {
                        PORT_FreeArena(dsap->pqg->arena, PR_TRUE);
                    }

                    if ((dsaKey != NULL) && (dsaKey->params.arena != NULL)) {
                ...
                case bltestECDSA:
                    if ((ecKey != NULL) && (ecKey->ecParams.arena != NULL)) {
                        PORT_FreeArena(ecKey->ecParams.arena, PR_TRUE);
                    }
