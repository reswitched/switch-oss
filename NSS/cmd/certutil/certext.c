...
static const char * const
basicConstraintKeyWordArray[] = { "noCaCert",  /*  Purposely 0 to make boolean logic easier!  */
                                  "caCert",
                                  NULL };

static SECStatus 
AddBasicConstraint(PLArenaPool *arena,
                   void *extHandle,
                   const char *userSuppliedValue)
{
    ...
    if (!userSuppliedValue ) {
        do {
            ...
            rv = SECU_EncodeAndAddExtensionValue(arena, extHandle,
                                                 &basicConstraint, isCriticalExt, SEC_OID_X509_BASIC_CONSTRAINTS,
                                                 (EXTEN_EXT_VALUE_ENCODER)CERT_EncodeBasicConstraintValue);
        } while (0);
    } else {
        /*  We need to process keywords as well as numbers which are for
            path length.  Values < 0 are unlimited paths.  */
        while (1) {
            pathValPos = nextPos;
            if (parseNextCmdInput(basicConstraintKeyWordArray,
                                  &value,
                                  &nextPos,
                                  &isCriticalExt) == SECFailure) {
                nextPos = strchr(pathValPos, ',');
                if (nextPos == NULL) {
                    pathIntLen = strlen(pathValPos);
                } else {
                    pathIntLen = nextPos - pathValPos;
                    nextPos += 1;
                }

                PORT_Memcpy(buffer, pathValPos, pathIntLen);
                buffer[pathIntLen] = 0;
                value = PORT_Atoi(buffer);

                /*  If decode failed, this is truely a bad argument, bail.  */
                if (value == 0) {
                    if (errno == EINVAL) {
                        return SECFailure;
                    }
                ...
                basicConstraint.pathLenConstraint = value;
            ...
            basicConstraint.isCA = (PRBool)value;
            if (!nextPos) {
                break;
            }
        }
              ...
              certutilExtnList extList, const char *extGeneric)
{
    ...
    do {
        ...
        if (extList[ext_basicConstraint].activated) {
            rv = AddBasicConstraint(arena,
                                    extHandle,
                                    extList[ext_basicConstraint].arg);
            if (rv) {
