...
#ifdef NN_NINTENDO_SDK
/*  In order to avoid issues with different compilers and NN macros, just
    define inline to be nothing on NNSDK.  */
#define inline
#else    /*  NN_NINTENDO_SDK  */
...
#ifdef NNSDK_ENABLE_FREEBL_DEV64
#define MP_MUL_DxD(a, b, Phi, Plo) NNSDK_MP_MUL_DxD(a, b, Phi, Plo)
#elif defined(MP_USE_UINT_DIGIT) && defined(MP_USE_LONG_LONG_MULTIPLY)
...
void s_mpv_mul_d_add_prop(const mp_digit *a, mp_size a_len, mp_digit b, mp_digit *c)
{
  ...
  mp_digit carry = 0;
#ifdef NNSDK_ENABLE_FREEBL_DEV64
  NNSDK_S_MPV_MUL_D_ADD_PROP(a, b, c, a_len, carry);
#else
  while (a_len--) {
    mp_digit a_i = *a++;
    mp_digit a0b0, a1b1;
...
#endif // NNSDK_ENABLE_FREEBL_DEV64
#endif
}
#endif

#ifdef NNSDK_ENABLE_FREEBL_DEV64
#define MP_SQR_D(a, Phi, Plo) NNSDK_MP_SQR_D(a, Phi, Plo)
#elif defined(MP_USE_UINT_DIGIT) && defined(MP_USE_LONG_LONG_MULTIPLY)
/* This trick works on Sparc V8 CPUs with the Workshop compilers. */
...
#ifdef NNSDK_ENABLE_FREEBL_DEV64
  NNSDK_MP_READ_UNSIGNED_OCTETS(mp, str, len);
  NNSDK_POSSIBLE_CODE_PATH_CHANGE_BY_RETURN();
#else
  mp_zero(mp);
#endif // NNSDK_ENABLE_FREEBL_DEV64
