...
static SECStatus _DecodePointer(void* dest,
                     const SEC_ASN1Template* templateEntry,
                     SECItem* src, PLArenaPool* arena, PRBool checkTag)
{
                     ...
                     SECItem* src, PLArenaPool* arena)
{
    ...
    {
        return _DecodePointer((void*)((char*)dest ),
                     ...
                     SECItem* src, PLArenaPool* arena)
{
    ...
    if (SECSuccess == rv)
    {
        if (templateEntry->kind & SEC_ASN1_POINTER)
        {
            rv = _DecodePointer(dest, templateEntry, &subItem, arena, PR_TRUE);
        }
        else
        {
                     ...
                     SECItem* src, PLArenaPool* arena, PRBool checkTag)
{
    ...
    if ((SECSuccess == rv) && (PR_TRUE == decode))
    {
        ...
        {
            rv = _DecodePointer(dest, templateEntry, &temp, arena, PR_TRUE);
        }
        else
        if (kind & SEC_ASN1_CHOICE)
        {
