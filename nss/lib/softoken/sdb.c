...
static const char *NNSDK_TMP_DIRS[] = {
    "/tmp",
    "/nss/tmp",
    NULL    /*  Must be the last entry  */
};

static char *sdb_getFallbackTempDir(void) {
    int                         i;
    char                        *ret = NULL;

    for (i = 0; i < PR_ARRAY_SIZE(NNSDK_TMP_DIRS); i++) {
        const char *curDir = NNSDK_TMP_DIRS[i];

        if (curDir == NULL) {
            continue;
        }

        if (PR_Access(curDir, PR_ACCESS_WRITE_OK) != PR_SUCCESS) {
            continue;
        }

        ret = PORT_Strdup(curDir);
        break;
    }
