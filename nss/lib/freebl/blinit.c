...
extern unsigned long nnsdk_getauxval(unsigned long type);
static unsigned long (*getauxval)(unsigned long) = nnsdk_getauxval;
#elif (__has_include(<sys/auxv.h>) || defined(__linux__)) && \
