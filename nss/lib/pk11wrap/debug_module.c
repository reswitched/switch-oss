...
print_final_statistics(void)
{
    ...
    if (fname) {
        ...
        out = PR_Open(fname,
                      ...
                      (PR_IRWXU | PR_IRWXG | PR_IRWXO));
    }
    if (!out) {
        out = PR_STDOUT;
    }
    PR_fprintf(out,"%-25s %10s %12s %12s %10s\n", "Function", "# Calls", 
               "Time", "Avg.", "% Time");
    PR_fprintf(out,"\n");
    ...
    PR_fprintf(out, "%-25s %10d %10d%2s ", nssdbg_prof_data[i].function,
               calls, prTime, type);
    /* for now always output the average in microseconds */
    PR_fprintf(out, "%10.2f%2s", (float)usTime / (float)calls, "us");
    PR_fprintf(out, "%10.2f%%", ((float)time / (float)total_time) * 100);
    PR_fprintf(out, "\n");
    }
    PR_fprintf(out, "\n");
