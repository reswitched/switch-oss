...
#ifdef PR_NO_DYN_LIBS
/*  On some platforms (NNSDK), no shared/dynamic library support is present
    so we statically "load" symbols for our usual shared libs.  */
extern SECStatus NSS_LibPreInit();
extern SECStatus NSS_LibPostInit();
extern void NSS_LibCleanup();
#endif
...
static const char *dllname =
	...
	/*  Name of the lib on NNSDK, but it is just a stub on the fs
            since loadable libs are not supported.  */
	"libnn_ckbi3.a";
#else
...
#ifdef PR_NO_DYN_LIBS
    /* On some platforms we may not have loadable library support, so
       call the lib pre-load to setup NSPR's link tables with the symbols
       we already have statically linked. */
    if (NSS_LibPreInit() != SECSuccess) {
        goto loser;
    }
#endif
...
#ifdef PR_NO_DYN_LIBS
    /* On some platforms we may not have loadable library support, so
       call the lib post-init to setup NSPR's link tables with the symbols
       we already have statically linked. */
    if (NSS_LibPostInit() != SECSuccess) {
        goto loser;
    }
#endif
