             ...
             PRBool ascii, secuPWData *pwdata)
{
    ...
    do {
        ...
        if (log) {
            if (log->head == NULL) {
                PR_fprintf(PR_STDOUT, "%s: certificate is valid\n", progName);
            ...
            if (rv != SECSuccess) {
                ...
                PR_fprintf(PR_STDOUT, "%s: certificate is invalid: %s\n",
...
#define FPS PR_fprintf(PR_STDOUT,
    ...
    FPS "\t%s -C [-c issuer-name | -x] -i cert-request-file -o cert-file\n"
        ...
        "\t\t [-1 | --keyUsage [keyUsageKeyword,..]]\n"
        "\t\t [-2 | --basicConstraint [basicConstraintKeyword,..]] [-3] [-4]\n"
...
luC(enum usage_level ul, const char *command)
{
    ...
    FPS "%-20s Create basic constraint extension. Possibe keywords and numbers:\n"
              "%-20s \"caCert\",\"noCaCert\",\"[-1,0..]\",\"critical\"\n",
        "   -2 | --basicConstraint keyword,number,keyword,...", "", "", "");
         ...
         int certVersion, PRBool pssSign, void *pwarg)
{
    ...
    if (rv != SECSuccess) {
        PR_fprintf(PR_STDERR, "Could not sign encoded certificate data.\n");
    ...
    SECItem *certDER)
{
    ...
    if (rv != SECSuccess) {
        ...
        PR_fprintf(PR_STDERR, "%s: unable to create cert (%s)\n", progName,
                SECU_Strerror(perr));
    }
    return (rv);
}
...
GetFlags(char *flagsString, flagArray *flags, int count)
{
    ...
    while (*flagsString) {
        ...
        if (i == count) {
            ...
            PR_fprintf(PR_STDERR, "Unknown flag (%s)\n", name);
...
DumpMergeLog(const char *progname, PK11MergeLog *log)
{
    ...
    for (node = log->head; node; node = node->next) {
        ...
        PR_fprintf(PR_STDERR, "%s: Could not merge object %s (type %s): %s\n",
                progName,
...
static const secuCommandFlag options_init[] =
    {
      ...
      { /* opt_GenericExtensions   */ 0, PR_TRUE, 0, PR_FALSE,
        "extGeneric" },
      { /* opt_AddBasicConstraintExt */ 0, PR_TRUE, 0, PR_FALSE,
        "basicConstraint" },
...
certutil_main(int argc, char **argv, PRBool initialize)
{
    ...
    if (certutil.commands[cmd_Version].activated) {
       PR_fprintf(PR_STDOUT, "Certificate database content version: command not implemented.\n");
    }
    ...
    if (certutil.commands[cmd_UpgradeMerge].activated) {
        ...
        if (!PK11_IsInternal(slot)) {
            PR_fprintf(PR_STDERR, "Only internal DB's can be upgraded\n");
            rv = SECSuccess;
            goto shutdown;
        }
        if (!PK11_IsRemovable(slot)) {
            PR_fprintf(PR_STDOUT, "database already upgraded.\n");
            rv = SECSuccess;
            goto shutdown;
        }
        if (!PK11_NeedLogin(slot)) {
            PR_fprintf(PR_STDOUT, "upgrade complete!\n");
            rv = SECSuccess;
            goto shutdown;
        }
        ...
        if (!PK11_IsPresent(slot)) {
             ...
             * info update */
            PR_fprintf(PR_STDERR, "upgrade/merge internal error\n");
            rv = SECFailure;
            goto shutdown;
        }
    ...
    if (certutil.commands[cmd_CreateAndAddCert].activated ||
        ...
        if (strstr(certutil.options[opt_Trust].arg, "u")) {
            PR_fprintf(PR_STDERR, "Notice: Trust flag u is set automatically if the "
...
int certutilMain(int argc, char **argv)
#else
int
