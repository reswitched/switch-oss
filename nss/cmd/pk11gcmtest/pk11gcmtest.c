    ...
    const unsigned char *aad, unsigned int aadlen, unsigned int tagsize)
{
    ...
    slot = NULL;
    if (!symKey) {
        PR_fprintf(PR_STDERR, "PK11_ImportSymKey failed\n");
        goto loser;
    }
    ...
    if (PK11_Encrypt(symKey, CKM_AES_GCM, &param,
        ...
        PR_fprintf(PR_STDERR, "PK11_Encrypt failed\n");
        goto loser;
    }
    ...
    const unsigned char *tag, unsigned int tagsize)
{
    ...
    if (inputlen + tagsize > sizeof(concatenated)) {
        PR_fprintf(PR_STDERR, "aes_decrypt_buf: local buffer too small\n");
        goto loser;
    }
...
static char *_privFileGetLine(char *buf, int bufSize, PRFileDesc *fd)
{
    char                        *ret = NULL;
    int                         i;
    int                         amt;
...
aes_gcm_kat(const char *respfn)
{
    ...
    if (strstr(respfn, "Encrypt") != NULL) {
        ...
        PR_fprintf(PR_STDERR, "Input file name must contain Encrypt or Decrypt\n");
        exit(1);
    }

    fd = PR_Open(respfn, PR_RDONLY, 0);
    if (fd == NULL) {
	PR_fprintf(PR_STDERR, "Cannot open input file %s\n", respfn);
	exit(1);
    }
    while (_privFileGetLine(buf, sizeof buf, fd) != NULL) {
        ...
        if (buf[0] == '[') {
            if (strncmp(&buf[1], "Keylen = ", 9) == 0) {
                ...
                if (test_group > 1) {
                    ...
                    PR_fprintf(PR_STDOUT, "%u tests\n", num_tests);
                }
                num_tests = 0;
                PR_fprintf(PR_STDOUT, "Keylen = %u, IVlen = %u, PTlen = %u, AADlen = %u, "
                       "Taglen = %u: ",
                ...
                PR_fprintf(PR_STDERR, "Unexpected input line: %s\n", buf);
                exit(1);
            }
            continue;
        }
        ...
        if (strncmp(buf, "Key", 3) == 0) {
            ...
            if (keysize != expected_keylen) {
                PR_fprintf(PR_STDERR, "Unexpected key length: %u vs. %u\n",
        ...
        if (strncmp(buf, "IV", 2) == 0) {
            ...
            if (ivsize != expected_ivlen) {
                PR_fprintf(PR_STDERR, "Unexpected IV length: %u vs. %u\n",
        ...
        if (strncmp(buf, "PT", 2) == 0) {
            ...
            if (plaintextlen != expected_ptlen) {
                PR_fprintf(PR_STDERR, "Unexpected PT length: %u vs. %u\n",
            ...
            if (!is_encrypt) {
                ...
                if (rv != SECSuccess) {
                    PR_fprintf(PR_STDERR, "aes_decrypt_buf failed\n");
                    goto loser;
                }
                if (outputlen != plaintextlen) {
                    PR_fprintf(PR_STDERR, "aes_decrypt_buf: wrong output size\n");
                    goto loser;
                }
                if (memcmp(output, plaintext, plaintextlen) != 0) {
                    PR_fprintf(PR_STDERR, "aes_decrypt_buf: wrong plaintext\n");
                    goto loser;
                }
            }
            continue;
        ...
        if (strncmp(buf, "FAIL", 4) == 0) {
            ...
            rv = aes_decrypt_buf(key, keysize, iv, ivsize,
                                 ...
                                 ciphertext, ciphertextlen, aad, aadlen, tag, tagsize);
            if (rv != SECFailure) {
                PR_fprintf(PR_STDERR, "aes_decrypt_buf succeeded unexpectedly\n");
                goto loser;
            }
            if (PORT_GetError() != SEC_ERROR_BAD_DATA) {
                PR_fprintf(PR_STDERR, "aes_decrypt_buf failed with incorrect "
                                "error code\n");
                goto loser;
            }
            continue;
        ...
        if (strncmp(buf, "AAD", 3) == 0) {
            ...
            if (aadlen != expected_aadlen) {
                PR_fprintf(PR_STDERR, "Unexpected AAD length: %u vs. %u\n",
        ...
        if (strncmp(buf, "CT", 2) == 0) {
            ...
            if (ciphertextlen != expected_ptlen) {
                PR_fprintf(PR_STDERR, "Unexpected CT length: %u vs. %u\n",
        ...
        if (strncmp(buf, "Tag", 3) == 0) {
            ...
            if (tagsize != expected_taglen) {
                PR_fprintf(PR_STDERR, "Unexpected tag length: %u vs. %u\n",
            ...
            if (is_encrypt) {
                ...
                if (rv != SECSuccess) {
                    PR_fprintf(PR_STDERR, "aes_encrypt_buf failed\n");
                    goto loser;
                }
                if (outputlen != plaintextlen + tagsize) {
                    PR_fprintf(PR_STDERR, "aes_encrypt_buf: wrong output size\n");
                    goto loser;
                }
                if (memcmp(output, ciphertext, plaintextlen) != 0) {
                    PR_fprintf(PR_STDERR, "aes_encrypt_buf: wrong ciphertext\n");
                    goto loser;
                }
                if (memcmp(output + plaintextlen, tag, tagsize) != 0) {
                    PR_fprintf(PR_STDERR, "aes_encrypt_buf: wrong tag\n");
                    goto loser;
                }
            }
            continue;
    ...
    PR_fprintf(PR_STDOUT, "%u tests\n", num_tests);
    PR_fprintf(PR_STDOUT, "%u test groups\n", test_group);
...
int pk11gcmtestMain(int argc, char **argv)
#else
