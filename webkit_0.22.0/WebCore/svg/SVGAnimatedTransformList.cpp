...
void SVGAnimatedTransformListAnimator::calculateAnimatedValue(float percentage, unsigned repeatCount, SVGAnimatedType* from, SVGAnimatedType* to, SVGAnimatedType* toAtEndOfDuration, SVGAnimatedType* animated)
{
    ...
    // ÅetoÅf attribute value, which conflicts mathematically with the requirement for additive transform animations
    // to be post-multiplied. As a consequence, in SVG 1.1 the behavior of to animations for ÅeanimateTransformÅf is undefined.
    const SVGTransformList& fromTransformList = from->transformList();
...
float SVGAnimatedTransformListAnimator::calculateDistance(const String& fromString, const String& toString)
{
    ...
    // Paced animations assume a notion of distance between the various animation values defined by the ÅetoÅf, ÅefromÅf, ÅebyÅf and ÅevaluesÅf attributes.
    // Distance is defined only for scalar types (such as <length>), colors and the subset of transformation types that are supported by ÅeanimateTransformÅf.
