...
public:
    ...
    virtual void didReceiveData(WebCore::ResourceHandle*, const char*, unsigned, int);
    ...
    virtual void willSendRequestAsync(WebCore::ResourceHandle*, WebCore::ResourceRequest&&, WebCore::ResourceResponse&&, WTF::CompletionHandler<void(WebCore::ResourceRequest&&)>&&);
    virtual void didReceiveResponseAsync(WebCore::ResourceHandle*, WebCore::ResourceResponse&&, WTF::CompletionHandler<void()>&&);
...
WKCDownloadPrivate::construct()
{
    ...
    WebCore::URL url = m_request.priv().webcore().url();
...
WKCDownloadPrivate::start()
{
    if (!m_resourceHandle) {
        ...
        if (m_view && m_view->mainFrame() && m_view->mainFrame()->core() && m_view->mainFrame()->core()->loader())
