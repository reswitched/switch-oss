...
NodePrivate::getElementsByTagName(const String& localName)
{
    ...
    RefPtr<WebCore::NodeList> list = ((WebCore::ContainerNode *)m_webcore)->getElementsByTagName(WTF::AtomicString(localName));
    if (!list)
        return 0;
...
NodePrivate::querySelector(const String& selectors)
{
    ...
    WebCore::Element* elem = ((WebCore::ContainerNode *)m_webcore)->querySelector(selectors).returnValue();
    if (!elem)
        return 0;
...
NodePrivate::getNodeCompositeRect(WKCRect* rect, int tx, int ty)
{
    ...
    const WebCore::RenderStyle& style = renderer->style();
    bool isOverflowXHidden = (style.overflowX() == WebCore::Overflow::Hidden);
    bool isOverflowYHidden = (style.overflowY() == WebCore::Overflow::Hidden);
