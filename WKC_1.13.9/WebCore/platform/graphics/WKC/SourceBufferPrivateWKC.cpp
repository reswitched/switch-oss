...
SourceBufferPrivateWKC::didReceiveSamples(const WKCMediaSample* in_samples, int in_samples_len)
{
    ...
    for (int i = 0; i < in_samples_len; i++) {
        m_client->sourceBufferPrivateDidReceiveSample(MediaSampleWKC::create(in_samples[i].fID, in_samples[i].fPresentationTime, in_samples[i].fDecodeTime, in_samples[i].fDuration, in_samples[i].fIsSync));
    }
}
