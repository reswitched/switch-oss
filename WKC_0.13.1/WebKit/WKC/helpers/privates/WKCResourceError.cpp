...
ResourceErrorPrivate::ResourceErrorPrivate(const ResourceError& error,
        ...
        const String& localizedDescription, ResourceHandle* resourceHandle)
    : m_webcore(new WebCore::ResourceError(domain, errorCode, failingURL,
        localizedDescription, resourceHandle ? resourceHandle->priv().webcore() : 0))
...
{
    m_private = new ResourceErrorPrivate(*this, domain, errorCode, failingURL, localizedDescription, resourceHandle);
    m_owned = true;
}
