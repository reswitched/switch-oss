...
#else
    WKC_DEFINE_STATIC_PTR(AtomicString*, defaultScriptType, 0);
    if (!defaultScriptType)
        defaultScriptType = new AtomicString("text/ecmascript");
    const AtomicString& type = fastGetAttribute(SVGNames::contentScriptTypeAttr);
    return type.isNull() ? *defaultScriptType : type;
#endif
}
...
#else
    WKC_DEFINE_STATIC_PTR(AtomicString*, defaultStyleType, 0);
    if (!defaultStyleType)
        defaultStyleType = new AtomicString("text/css");
    const AtomicString& type = fastGetAttribute(SVGNames::contentStyleTypeAttr);
    return type.isNull() ? *defaultStyleType : type;
#endif
}
