...
#if PLATFORM(WKC)
template<> inline
std::unique_ptr<SVGAnimatedInteger::ContentType> SVGAnimatedTypeAnimator::constructFromBaseValue<SVGAnimatedInteger>(const SVGElementAnimatedPropertyList& animatedTypes)
{
    ASSERT(animatedTypes[0].properties.size() == 1);
    const int& animatedType = castAnimatedPropertyToActualType<SVGAnimatedInteger>(animatedTypes[0].properties[0].get())->currentBaseValue();

    void* p = fastMalloc(sizeof(int));
    auto copy = std::unique_ptr<SVGAnimatedInteger::ContentType>(new (p) int(animatedType));
    executeAction<SVGAnimatedInteger>(StartAnimationAction, animatedTypes, 0, copy.get());
    return copy;
}
#endif
