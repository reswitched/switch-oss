...
static SVGPropertyInfo* createSVGTestPropertyInfo(const QualifiedName& attributeName, SVGPropertyInfo::SynchronizeProperty synchronizeFunction)
{
    return new SVGPropertyInfo(AnimatedUnknown, PropertyIsReadWrite, attributeName, attributeName.localName(), synchronizeFunction, nullptr);
}

static SVGAttributeToPropertyMap* createSVGTextAttributeToPropertyMap()
{
    SVGAttributeToPropertyMap* map = new SVGAttributeToPropertyMap();

    WKC_DEFINE_STATIC_PTR(SVGPropertyInfo*, requiredFeatures, createSVGTestPropertyInfo(requiredFeaturesAttr, SVGElement::synchronizeRequiredFeatures));
    map->addProperty(*requiredFeatures);

    WKC_DEFINE_STATIC_PTR(SVGPropertyInfo*, requiredExtensions, createSVGTestPropertyInfo(requiredExtensionsAttr, SVGElement::synchronizeRequiredExtensions));
    map->addProperty(*requiredExtensions);

    WKC_DEFINE_STATIC_PTR(SVGPropertyInfo*, systemLanguage, createSVGTestPropertyInfo(systemLanguageAttr, SVGElement::synchronizeSystemLanguage));
    map->addProperty(*systemLanguage);

    return map;
}
const SVGAttributeToPropertyMap& SVGTests::attributeToPropertyMap()
{
    WKC_DEFINE_STATIC_PTR(SVGAttributeToPropertyMap*, map, createSVGTextAttributeToPropertyMap());
    return *map;
}
#endif
