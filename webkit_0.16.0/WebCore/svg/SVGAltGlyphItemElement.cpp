...
bool SVGAltGlyphItemElement::hasValidGlyphElements(Vector<String>& glyphNames) const
{
    ...
    // The ÅealtGlyphItemÅf element defines a candidate set of possible glyph substitutions.
    // The first ÅealtGlyphItemÅf element whose referenced glyphs are all available is chosen.
    // Its glyphs are rendered instead of the character(s) that are inside of the referencing
    // ÅealtGlyphÅf element.
    //
