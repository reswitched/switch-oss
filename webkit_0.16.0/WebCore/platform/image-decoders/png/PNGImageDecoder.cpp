...
#else
    void createInterlaceBuffer(int size) { m_interlaceBuffer = (png_byte *)WTF::fastMalloc(sizeof(png_byte)*size); }
#endif
#if USE(QCMSLIB)
    ...
    if (buffer.status() == ImageFrame::FrameEmpty) {
        ...
        if (m_currentFrame) {
            if (!initFrameBuffer(m_currentFrame)) {
                buffer.setStatus(ImageFrame::FrameEmpty);
                longjmp(JMPBUF(png), 1);
                return;
            }
        }
...
void PNGImageDecoder::decode(bool onlySize, unsigned haltAtFrame)
{
    ...
    if (!m_data->needsReallocationToMergeBuffers(&bytesToBeAllocated)
        ...
        failed = !m_reader->decode(*m_data, onlySize, haltAtFrame) && isAllDataReceived();
    }
    else {
        failed = true;
        wkcMemoryNotifyMemoryAllocationErrorPeer(bytesToBeAllocated, WKC_MEMORYALLOC_TYPE_IMAGE_SHAREDBUFFER);
    }
    if (failed)
        setFailed();
#else
