...
#else
        m_medium = std::make_unique<MediaQueryEvaluator>(String("all"));
#endif
    ...
    StyleSharingBehavior sharingBehavior, RuleMatchingBehavior matchingBehavior, const RenderRegion* regionForStyling)
{
    ...
    if (sharingBehavior == AllowStyleSharing && !element->document().haveStylesheetsLoaded() && !element->renderer()) {
        if (!s_styleNotYetAvailable) {
...
#if PLATFORM(WKC)
            // FIX : https://bugs.webkit.org/show_bug.cgi?id=158297
            ...
            s_styleNotYetAvailable->setFontDescription(fontDescription);
#endif
...
#else
WKC_DEFINE_GLOBAL_BOOL(inLoadPendingResources, false);
#endif
#endif
