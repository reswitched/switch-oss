...
void RenderSVGRoot::computeIntrinsicRatioInformation(FloatSize& intrinsicSize, double& intrinsicRatio) const
{
    ...
    // The intrinsic aspect ratio of the viewport of SVG content is necessary for example, when including SVG from an ÅeobjectÅf
    ...
    // - If the ÅewidthÅf and ÅeheightÅf of the rootmost ÅesvgÅf element are both specified with unit identifiers (in, mm, cm, pt, pc,
    //   px, em, ex) or in user units, then the aspect ratio is calculated from the ÅewidthÅf and ÅeheightÅf attributes after
    ...
    if (!intrinsicSize.isEmpty())
        ...
        // - If either/both of the ÅewidthÅf and ÅeheightÅf of the rootmost ÅesvgÅf element are in percentage units (or omitted), the
        //   aspect ratio is calculated from the width and height values of the ÅeviewBoxÅf specified for the current SVG document
        //   fragment. If the ÅeviewBoxÅf is not correctly specified, or set to 'none', the intrinsic aspect ratio cannot be
